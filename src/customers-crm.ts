...
function cusRow(c) {
  ...
  return '<div class="trow km krow crow' + (on ? " sel" : "") + '" role="link" tabindex="0"' +
    ...
    '<div class="c-name" style="display:flex;align-items:center;gap:12px;min-width:0;">' +
      '<span role="img" aria-label="' + (c.test ? "جهة تجريبية" : "جهة فعلية") + '" style="width:9px;height:9px;border-radius:999px;flex:none;background:' + (c.test ? "#D8DCE3" : "#2563EB") + ';"></span>' +
      ...
      (c.waName ? '<span style="font-size:12px;color:#656B76;flex:none;direction:ltr;">' + esc(c.phone) + '</span>' : "") + '</div>' +
    '<div class="c-meta">' +
      '<div class="c-prod taglvl" title="' + esc(oc.hint) + '"><span class="d" style="background:' + oc.dot + ';"></span><span style="font-size:14px;color:#33373E;">' + oc.label + '</span></div>' +
      '<div class="c-state">' + (c.human ? '<span style="font-size:12px;color:#7A5600;">تدخّل بشري</span>' :
        (since ? '<span style="font-size:12px;color:#656B76;">منذ ' + fmtAgo(Date.now() - since) + '</span>' : "")) + '</div>' +
    '</div>' +
    '<div class="c-fig fig">' +
      '<div class="c-num" style="text-align:start;">' + (lvl
        ? '<span class="taglvl"><span class="d" style="background:' + lvl.dot + ';"></span><span style="font-size:14px;color:#33373E;">' + lvl.label + '</span></span>' +
          '<span style="font-size:12px;color:#656B76;"> · ' + esc(tag.product) + '</span>'
        : '<span style="font-size:14px;color:#656B76;">—</span>') + '</div>' +
      '<div class="c-num">' + fmtN(msgs) + '</div>' +
      '<div class="c-num" style="text-align:start;font-weight:450;color:#656B76;font-size:12px;">' + (c.lastEventAt ? fmtD(c.lastEventAt) : "—") + '</div>' +
    '</div>' +
    '<div class="c-act" style="text-align:center;"><span style="color:#656B76;font-size:14px;">‹</span></div>' +
  '</div>';
}
...
function cusHeader(allOn) {
  return '<div class="crow thead-wide" style="padding:8px 20px 8px 12px;background:#fff;border-bottom:1px solid #ECEEF2;font-size:12px;font-weight:500;color:#656B76;">' +
    ...
    '<div class="c-fig fig">' +
      '<div class="c-num" style="text-align:start;color:#656B76;font-size:12px;">اهتمام المساعد</div>' +
      '<div class="c-num" style="color:#656B76;font-size:12px;">الرسائل</div>' +
      '<div class="c-num" style="text-align:start;color:#656B76;font-size:12px;">آخر نشاط</div>' +
    '</div>' +
    '<div></div></div>' +
...
function cusControlBar(nTotal) {
  ...
  if (tags.length) {
    h += '<select onchange="cusSetTag(this.value)" class="crmsel"' +
      (cusTagF ? ' style="border-color:#5B8DEF;color:#2563EB;background:#DCE8FC;"' : "") + '>' +
...
function cusGroupView(rows) {
  ...
  if (order.length <= 1) {
    return '<div class="sparse rise">' + ic("eye", 16, "#2563EB") +
      ...
      '<span class="lnk" onclick="cusSetView(&quot;list&quot;)" style="color:#2563EB;font-weight:500;cursor:pointer;">→ عد إلى القائمة</span></div></div>' + cusListView(rows);
  }
  var h = "";
  order.forEach(function (k) {
    ...
    h += '<div class="tblwrap crmflat cusflat rise" style="margin-bottom:14px;">' +
      '<div style="display:flex;align-items:center;gap:9px;padding:8px 20px 8px 12px;border-bottom:1px solid #ECEEF2;background:#EFF1F5;">' +
      '<span style="font-size:14px;font-weight:500;color:#14161A;">' + esc(k) + '</span>' +
    ...
    h += '</div></div>';
    if (by[k].length > g.length) h += '<div class="tfoot"><span style="color:#7A5600;font-weight:500;">تُعرض ' + fmtN(g.length) + " من " + fmtN(by[k].length) + '</span></div>';
    h += '</div>';
  });
  return h;
}
...
function vCustomersCrm() {
  ...
  if (!((cache && cache.contacts) || []).length) {
    return '<div class="empty" style="padding:60px 20px;"><div class="ic"><span></span></div>' +
      ...
      '<a href="#targets" style="color:#2563EB;font-weight:500;">جهات الاستهداف</a> — كل جهة تظهر هنا بعد أول رسالة.</div></div>';
  }
