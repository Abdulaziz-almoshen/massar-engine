...
<style>
  /* ===================== BRAND TOKENS — DESIGN.md is the authority =====================
     The Seha design system approved 2026-09-03. Before this block existed the palette lived as
     1,299 hardcoded hexes across src/, which is why the rebrand had to be a mechanical remap:
     there was no single place to change. There is now. New code uses var(--x); a raw hex in a
     component is a bug unless DESIGN.md says otherwise (WhatsApp's own bubble colours are the
     documented exception, since they quote another product's surface).
     ==================================================================================== */
  :root {
    --blue:#2563EB; --blue-deep:#1E5FCC; --blue-light:#5B8DEF;
    --blue-tint:#EAF1FE; --blue-wash:#DCE8FC;
    /* Both stops must carry white text: 7.81:1 on #1A47BE and 5.17:1 on #2563EB. --blue-light
       (#5B8DEF) is NOT in the ramp: white on it is 2.86:1, so it stays decorative and is never a
       text ground and never a mark on a tinted row. */
    --grad:linear-gradient(270deg,#1A47BE,#2563EB);
    /* The tinted ACTION ROW ground — Tonomo's most distinctive control: a full-width bar in
       accent-tint carrying an accent-coloured label and icon, used for add/manage actions
       that are neither primary buttons nor plain links. */
    --accent-bar:#EAF1FE; --accent-bar-hover:#DCE8FC;
    ...
    --s-sched:#5B8DEF;  --s-sched-soft:#E9F0FE;  --s-sched-text:#1A47BE;
    --s-review:#1E5FCC; --s-review-soft:#E7EEFB; --s-review-text:#173FA8;
    --s-attend:#2563EB; --s-attend-soft:#EAF1FE; --s-attend-text:#1A47BE;
    --s-off:#A2A9B4;    --s-off-soft:#EEF0F3;    --s-off-text:#464C56;
    ...
    --s-attn-mark:#B37F00;  --s-sched-mark:#4A7BE8;  --s-off-mark:#767D89;
    ...
    --sh-3:0 3px 6px -4px rgba(16,24,40,.10),0 8px 20px rgba(16,24,40,.10),0 12px 32px 8px rgba(16,24,40,.06);
  ...
  /* interior.dev, failure 3: "Motion is the only channel." With reduced motion on, the
     information still has to arrive — the trip is optional, the destination is not. Every surface
     must be correct and complete with every transition removed. */
  ...
  * { box-sizing: border-box; }
  ...
  body { font-family: 'Cairo', system-ui, 'Segoe UI', 'Geeza Pro', Tahoma, 'Noto Naskh Arabic', sans-serif; background: #FFFFFF; color: #14161A; font-size: 14px; font-weight: 450; line-height: 1.45; letter-spacing: 0; }
  ::selection { background: #5B8DEF; color: #fff; }
  .ms-scroll::-webkit-scrollbar { height: 8px; width: 8px; }
  .ms-scroll::-webkit-scrollbar-thumb { background: #A2A9B4; border-radius: 999px; }
  ...
  aside { width: 250px; flex: none; background: #EFF1F5; color: #33373E; display: flex;
    flex-direction: column; border-inline-end: 1px solid #ECEEF2; }
  .switcher { display:flex; align-items:center; gap:10px; width:100%; font-family:inherit;
    ...
    border-bottom:1px solid #ECEEF2; cursor:pointer; text-align:start; }
  .switcher:hover { background:#E5E8EE; }
  .switcher .chev { color:#656B76; font-size:12px; flex:none; }
  .switcher .logo { width:28px; height:28px; flex:none; border-radius:6px;
    background:linear-gradient(135deg,#5B8DEF,#2563EB); display:flex; align-items:center;
    justify-content:center; font-weight:600; font-size:14px; color:#fff; }
  .switcher .t1 { font-size:14px;
    font-weight:600; color:#14161A; line-height:1.3; }
  .switcher .t2 { font-size:12px; font-weight:450; color:#656B76; margin-top:1px;
    overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
  nav { flex:1; min-height:0; overflow-y:auto; padding:8px; }
  .grp { font-size:12px; font-weight:500; color:#656B76; padding:6px 10px; margin-block-start:12px; }
  ...
  .nv { display:flex; align-items:center; gap:10px; width:100%; font-family:inherit; height:32px;
    font-size:14px; font-weight:450; color:#33373E; background:transparent; border:none;
    border-radius:6px; padding-inline:10px; cursor:pointer; text-align:start; margin-bottom:1px; }
  .nv:hover { background:#E5E8EE; }
  .nv.on { font-weight:500; color:#14161A; background:#ECEEF2; }
  .nv .gx { width: 20px; height: 20px; flex: none; display: flex; align-items: center; justify-content: center; }
  .nv .lbl { flex: 1; }
  .nv .dot { display:none; }
  .g-sq { width: 13px; height: 13px; border-radius: 3px; background: #A2A9B4; }
  .g-ci { width: 13px; height: 13px; border-radius: 999px; background: #A2A9B4; }
  .g-di { width: 11px; height: 11px; background: #A2A9B4; transform: rotate(45deg); border-radius: 2px; }
  .g-tr { width: 0; height: 0; border-right: 7px solid transparent; border-left: 7px solid transparent; border-bottom: 12px solid #A2A9B4; }
  .g-ba { width: 13px; height: 13px; border-right: 3px solid #A2A9B4; border-left: 3px solid #A2A9B4; border-radius: 1px; }
  .g-ri { width: 13px; height: 13px; border-radius: 999px; border: 3px solid #A2A9B4; }
  .g-tb { width: 13px; height: 13px; border-top: 3px solid #A2A9B4; border-bottom: 3px solid #A2A9B4; }
  .g-tree { width: 13px; height: 13px; border: 2px solid #A2A9B4; border-radius: 3px; }
  .nv.on .gx > * { background-color:#33373E; border-color:#33373E; }
  .nv.on .g-tr { background:none; border-bottom-color:#33373E; }
  ...
  .collapse { display:flex; align-items:center; gap:10px; font-family:inherit; height:32px;
    margin:8px; width:calc(100% - 16px); padding-inline:10px; font-size:12px; color:#656B76;
    background:transparent; border:none; border-radius:6px; cursor:pointer; text-align:start;
    flex:none; }
  .collapse:hover { background:#E5E8EE; }
  .collapse .cicon { color:#656B76; font-size:14px; }
  ...
  header.crumb { height:42px; flex:none; display:flex; align-items:center; gap:8px;
    padding-inline:20px; background:#fff; border-bottom:1px solid #ECEEF2; }
  header.crumb .t { font-size:14px; font-weight:500; color:#14161A; }
  header.crumb .sep { font-size:14px; color:#656B76; }
  header.crumb .s { font-size:14px; font-weight:450; color:#33373E; }
  ...
  .subnav { position:relative; height:36px; flex:none; display:flex; align-items:stretch; gap:2px; padding-inline:20px;
    background:#fff; border-bottom:1px solid #ECEEF2; overflow-x:auto; scrollbar-width:none; }
  ...
  /* transitions.dev «Tabs sliding»: the indicator FOLLOWS the active tab instead of cutting to it.
     ...
     and width only — both composite. It is positioned from JS because CSS cannot measure a
     sibling. */
  .subnav .ind { position:absolute; inset-block-end:0; inset-inline-start:0; height:2px;
    background:var(--blue); border-radius:var(--r-pill) var(--r-pill) 0 0; width:0;
    transform:translateX(0); transition:transform var(--base) var(--ease), width var(--base) var(--ease);
    pointer-events:none; }
  ...
  .subnav .sub.soon { color:#656B76; }
  .subnav .sub.soon:hover { color:#656B76; }
  ...
  .subnav .sub:focus-visible { outline:2px solid #2563EB; outline-offset:-2px; }
  ...
  header .t { font-size: 22px; font-weight: 600; color: #14161A; letter-spacing: 0; }
  header .s { font-size: 12px; color: #656B76; margin-top: 3px; }
  .livechip { display:inline-flex; align-items:center; gap:6px; font-size:12px; color:#656B76;
    background:transparent; padding:0; border-radius:0; }
  .livechip .d { width: 7px; height: 7px; border-radius: 999px; background: #5B8DEF; }
  ...
  .kpi { background: #fff; border: 1px solid #ECEEF2; border-radius: 10px; padding: 15px 17px; display: flex; flex-direction: column; gap: 7px; }
  .kpi .k { font-size: 12px; color: #656B76; font-weight: 450; }
  .kpi .v { font-size: 22px; font-weight: 600; color: #14161A; line-height: 1.1; font-variant-numeric: tabular-nums; letter-spacing: 0; }
  ...
  .affin { background: #EFF1F5; border: 1px solid #ECEEF2; border-radius: 12px; padding: 14px 16px; margin-bottom: 16px; }
  .affin .ah { display: flex; align-items: baseline; gap: 10px; flex-wrap: wrap; margin-bottom: 4px; }
  .affin .ah .t { font-size: 14px; font-weight: 500; color: #14161A; }
  .affin .ah .s { font-size: 12px; color: #656B76; }
  ...
  .affin .fld > span { font-size: 12px; color: #656B76; white-space: nowrap; }
  .affin select { font-family: inherit; height: 32px; border: 1px solid #D8DCE3; border-radius: 6px;
    background: #fff; color: #14161A; font-size: 12px; padding: 0 10px; cursor: pointer; max-width: 210px; }
  .affin select.on { border-color: #2563EB; color: #2563EB; background: #EAF1FE; }
  .excl { font-family: inherit; font-size: 12px; font-weight: 500; border-radius: 999px; padding: 7px 14px;
    cursor: pointer; border: 1px solid #D8DCE3; background: #fff; color: #33373E; white-space: nowrap;
  ...
  .aq:hover { background: #EFF1F5; }
  .aqd { width: 8px; height: 8px; border-radius: 999px; flex: none; }
  .aqav { width: 32px; height: 32px; flex: none; border-radius: 8px; background: #E5E8EE; color: #33373E; display: flex; align-items: center; justify-content: center; font-weight: 500; font-size: 14px; }
  .aqic { background: #EFF1F5; }
  .aqt { flex: 0 0 min(42%, 360px); min-width: 0; display: flex; flex-direction: column; gap: 3px; }
  .aqn { font-size: 14px; font-weight: 500; color: #14161A; display: flex; align-items: center; gap: 6px; flex-wrap: wrap; }
  .aqw { font-size: 12px; color: #656B76; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
  .noact .aqa { display: none; }
  .noact .aqt { flex: 1 1 auto; }
  .aqa { flex: 1 1 auto; min-width: 0; font-size: 12px; color: #33373E; line-height: 1.55; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; }
  .aqgo { flex: none; font-size: 12px; font-weight: 500; color: #33373E; opacity: 0; transition: opacity .14s ease; }
  ...
  .kbrow { display: grid; grid-template-columns: minmax(0,1.8fr) 1fr minmax(0,1.4fr) .55fr .7fr 52px;
    align-items: center; gap: 14px; padding: 12px 20px 12px 12px; border-top: 1px solid #ECEEF2;
    text-decoration: none; transition: background .14s ease; }
  .kbrow:first-of-type { border-top: 0; }
  a.kbrow:hover { background: #EFF1F5; }
  .kbrow .nm { font-size: 14px; font-weight: 450; color: #14161A; display: flex; align-items: center;
    gap: 7px; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
  .kbrow .st { display: flex; align-items: center; gap: 7px; font-size: 12px; color: #33373E; min-width: 0; }
  .kbrow .st .d { width: 6px; height: 6px; border-radius: 999px; flex: none; }
  .kbrow .st .fn { direction: ltr; color: #656B76; font-size: 12px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
  .kbrow .fig { font-size: 14px; color: #14161A; font-variant-numeric: tabular-nums; }
  .kbrow .go { font-size: 12px; font-weight: 500; color: #33373E; opacity: 0; transition: opacity .14s ease; }
  ...
  .oprow { display: grid; grid-template-columns: 28px minmax(0,1.5fr) 1.05fr minmax(0,2fr) 52px;
    align-items: center; gap: 12px; padding: 11px 20px 11px 12px; border-top: 1px solid #ECEEF2;
    cursor: pointer; transition: background .14s ease; }
  .oprow:first-of-type { border-top: 0; }
  .oprow:hover { background: #EFF1F5; }
  .oprow .av { width: 28px; height: 28px; border-radius: 7px; background: #E5E8EE; color: #33373E;
    display: flex; align-items: center; justify-content: center; font-size: 12px; font-weight: 500; }
  .oprow .nm { font-size: 14px; font-weight: 450; color: #14161A; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
  .oprow .ph { font-size: 12px; color: #656B76; direction: ltr; text-align: start; font-variant-numeric: tabular-nums; }
  .oprow .wh { min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
  .oprow .go { font-size: 12px; font-weight: 500; color: #33373E; opacity: 0; transition: opacity .14s ease; }
  ...
  .card { background:#fff; border:1px solid #ECEEF2; border-radius:10px; padding:16px;
    margin-bottom:16px; }
  .card h3 { margin: 0 0 16px; font-size: 14px; font-weight: 600; color: #33373E; letter-spacing: 0; }
  .chip { display:inline-flex; align-items:center; gap:6px; font-size:12px; font-weight:450;
    border-radius:6px; padding:3px 8px; white-space:nowrap; background:#EFF1F5;
    border:1px solid #ECEEF2; color:#33373E; }
  /* Semantic classes now tint only the TEXT, never the fill. */
  .c-grey { color:#33373E; } .c-blue { color:#1E5FCC; }
  .c-teal { color:#2563EB; } .c-ok { color:#12633F; }
  ...
  .ptab { font-family:inherit; font-size:12px; font-weight:450; border:1px solid #ECEEF2;
    background:#fff; color:#33373E; border-radius:6px; padding:6px 12px; cursor:pointer; }
  .ptab.on { background:#ECEEF2; color:#14161A; border-color:#ECEEF2; }
  .inp { font-family: inherit; font-size: 14px; color: #14161A; border: 1px solid #D8DCE3; border-radius: 12px; padding: 11px 16px; background: #fff; outline: none; }
  .inp:focus { border-color: #2563EB; box-shadow: 0 0 0 3px rgba(31,122,115,.15); }
  .fun { margin-bottom: 13px; }
  .fun .r1 { display: flex; align-items: baseline; justify-content: space-between; gap: 12px; margin-bottom: 6px; }
  .fun .l { font-size: 12px; font-weight: 600; color: #14161A; }
  .fun .m { font-size: 12px; color: #656B76; font-variant-numeric: tabular-nums; }
  .fun .track { height: 9px; background: #E5E8EE; border-radius: 999px; overflow: hidden; }
  ...
  .empty .ic span { width: 26px; height: 26px; border: 2px dashed #D8DCE3; border-radius: 7px; }
  .empty .t { font-size: 16px; font-weight: 600; color: #14161A; }
  .empty .s { font-size: 14px; color: #656B76; margin-top: 6px; max-width: 380px; line-height: 1.8; }

  /* tables */
  .tblwrap { background: #fff; border: 1px solid #ECEEF2; border-radius: 16px; overflow: hidden; margin-bottom: 18px; box-shadow: 0 1px 2px rgba(16,24,40,.04); }
  .ttoolbar { display: flex; align-items: center; gap: 10px; flex-wrap: wrap; padding: 18px 22px; border-bottom: 1px solid #ECEEF2; }
  .cntpill { font-size: 12px; font-weight: 600; color: #2563EB; background: #EAF1FE; border-radius: 999px; padding: 4px 12px; }
  ...
  .sparse { display: flex; align-items: flex-start; gap: 10px; margin: -6px 0 16px; padding: 12px 16px; border: 1px solid #D8DCE3; border-inline-start: 3px solid #2563EB; border-radius: 12px; background: #fff; font-size: 12px; line-height: 1.85; color: #33373E; }
  .sparse b { color: #14161A; font-weight: 600; }
  .sparse .lnk { color: #2563EB; font-weight: 600; cursor: pointer; text-decoration: none; }
  .tfoot { display: flex; align-items: center; justify-content: space-between; gap: 12px; flex-wrap: wrap; padding: 12px 20px; border-top: 1px solid #ECEEF2; background: #EFF1F5; font-size: 12px; color: #656B76; }
  ...
  .pgnav button, .pgsize { font-family: inherit; font-size: 12px; color: #33373E; background: #fff;
    border: 1px solid #D8DCE3; border-radius: 6px; height: 28px; padding: 0 10px; cursor: pointer;
    transition: background .14s ease, border-color .14s ease; }
  .pgnav button:hover:not(:disabled), .pgsize:hover { background: #E5E8EE; border-color: #656B76; }
  .pgnav button:disabled { color: #656B76; cursor: default; background: #EFF1F5; }
  .pgnav .at { font-variant-numeric: tabular-nums; color: #14161A; padding: 0 4px; white-space: nowrap; }
  .pgrange { font-variant-numeric: tabular-nums; color: #33373E; }
  .pgrange b { color: #14161A; font-weight: 500; }
  .pgbtn { width: 34px; height: 34px; border-radius: 999px; border: 1px solid #ECEEF2; background: #fff; color: #33373E; font-family: inherit; font-size: 12px; font-weight: 600; cursor: pointer; }
  .pgbtn.on { background: #14161A; color: #fff; border-color: #14161A; }
  .kebab { width: 32px; height: 32px; border-radius: 8px; border: none; background: transparent; color: #656B76; font-size: 16px; cursor: pointer; line-height: 1; }
  .kebab:hover { background: #E5E8EE; color: #33373E; }
  .swt { width: 38px; height: 22px; border-radius: 999px; background: #ECEEF2; position: relative; flex: none; transition: background .18s ease; }
  .swt.on { background: #2563EB; }
  ...
  .thead { background: #EFF1F5; border-bottom: 1px solid #ECEEF2; font-size: 12px; font-weight: 600; color: #656B76; }
  .trow { border-bottom: 1px solid #E5E8EE; cursor: pointer; min-height: 62px; }
  .trow:hover { background: #EFF1F5; }
  .trow:last-child { border-bottom: none; }
  .cust { display: flex; align-items: center; gap: 12px; min-width: 0; }
  .cust .av { width: 40px; height: 40px; flex: none; border-radius: 999px; background: #E5E8EE; display: flex; align-items: center; justify-content: center; color: #33373E; font-weight: 600; font-size: 14px; }
  .cust .nm { font-size: 14px; font-weight: 600; color: #14161A; }
  .cust .ph { font-size: 12px; color: #656B76; direction: ltr; text-align: right; }
  .lastm { font-size: 12px; color: #656B76; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .tm { font-size: 12px; color: #656B76; font-variant-numeric: tabular-nums; }
  .statgrid { display: grid; grid-template-columns: repeat(auto-fit, minmax(130px, 1fr)); gap: 14px; margin-bottom: 18px; }
  .statc { background:#fff; border:1px solid #ECEEF2; border-radius:10px; padding:14px 16px; }
  .statc .l { font-size: 12px; color: #656B76; margin-bottom: 8px; font-weight: 600; }
  .statc .v { font-size: 22px; font-weight: 600; color: #14161A; line-height: 1; font-variant-numeric: tabular-nums; }
  .statc .p { font-size: 12px; color: #2563EB; font-weight: 600; margin-top: 6px; }
  .statc .mb { height: 4px; background: #E5E8EE; border-radius: 999px; overflow: hidden; margin-top: 9px; }
  ...
  .convo .hd { flex: none; display: flex; align-items: center; gap: 11px; padding: 14px 18px; border-bottom: 1px solid #ECEEF2; }
  .convo .hd .av { width: 40px; height: 40px; flex: none; border-radius: 999px; background: #E5E8EE; color: #33373E; display: flex; align-items: center; justify-content: center; font-weight: 600; font-size: 16px; }
  .convo .msgs { flex: 1; overflow-y: auto; background: #E5DDD4; padding: 16px; }
  .convo .ft { flex: none; padding: 13px 18px; border-top: 1px solid #ECEEF2; }
  ...
  .bub { max-width: 76%; border-radius: 12px; padding: 9px 13px; font-size: 12px; line-height: 1.9; margin-bottom: 9px; box-shadow: 0 1px 1px rgba(0,0,0,.06); white-space: pre-line; color: #14161A; }
  ...
  .step .num { width: 32px; height: 32px; flex: none; border-radius: 999px; display: flex; align-items: center; justify-content: center; font-size: 14px; font-weight: 600; background: #1E5FCC; color: #fff; }
  .step .num.done { background: #EAF1FE; color: #2563EB; }
  .step .ht { font-size: 14px; font-weight: 600; color: #14161A; }
  .step .hs { font-size: 12px; color: #656B76; margin-top: 4px; }
  ...
  .prod .bar { height: 6px; background: #E5E8EE; border-radius: 999px; overflow: hidden; margin: 10px 0; }
  ...
  .btn-dark { color: #fff; background: #14161A; }
  .btn-ghost { color: #14161A; background: #fff; border: 1px solid #D8DCE3; }
  .btn:hover { filter: brightness(.97); }
  .btn-dis { color: #656B76; background: #E5E8EE; cursor: not-allowed; }
  .note { display:flex; align-items:center; gap:9px; background:#fff; border:1px solid #ECEEF2;
    border-inline-start:3px solid #7A5600; border-radius:10px; padding:12px 16px; font-size:12px;
    color:#33373E; font-weight:450; margin-top:14px; }

  /* kb */
  .kbrow { display: flex; align-items: center; gap: 14px; padding: 17px 22px; border-bottom: 1px solid #E5E8EE; }
  ...
  .kbrow .t1 { font-size: 14px; font-weight: 600; color: #14161A; }
  .kbrow .t2 { font-size: 12px; color: #656B76; margin-top: 4px; }
  .kbrow .ct { font-size: 12px; color: #656B76; }
  ...
  .gate input { font-family: inherit; width: 100%; font-size: 14px; border: 1px solid #D8DCE3; border-radius: 12px; padding: 12px 14px; margin: 14px 0; direction: ltr; }
  .ptitle { display: flex; align-items: flex-start; gap: 16px; flex-wrap: wrap; margin-bottom: 22px; }
  .ptitle h1 { margin:0; font-size:18px; font-weight:600; color:#14161A; letter-spacing:0;
    line-height:1.4; }
  .ptitle p { margin: 6px 0 0; font-size: 14px; color: #656B76; }
  .ptitle .acts { margin-inline-start: auto; display: flex; gap: 10px; align-items: center; }
  .sec { font-size: 14px; font-weight: 600; color: #33373E; margin: 4px 0 14px; }
  /* motion */
  ...
  .skel { background: linear-gradient(90deg, #E5E8EE 25%, #ECEEF2 37%, #E5E8EE 63%); background-size: 200% 100%; animation: shimmer 1.4s linear infinite; border-radius: 8px; }
  .sec .meta { font-size: 12px; font-weight: 600; color: #656B76; margin-inline-start: 8px; }
  ...
  @media (max-width: 430px) {
    ...
    .lbar .btn { padding: 10px 16px !important; font-size: 12px !important; }
  }
  ...
  @media (max-width: 900px) {
    .convled { width: 100% !important; border-inline-start: 0 !important; padding-inline-start: 0 !important;
               border-top: 1px solid #E5E8EE; padding-top: 14px; }
  }
  ...
  .pm-h { background: #2563EB; border-radius: 2px; }
  .pm-a { border: 1.5px dashed #7A5600; border-radius: 999px; background: transparent; }
  .pm-i { background: #1E5FCC; border-radius: 2px; opacity: .55; }
  .pm-m { border: 1px dashed #D8DCE3; border-radius: 2px; background: transparent; }
  /* EXACT-1: two columns, 35/65, not a stacked pair. min-w-20 = 80px floor on the label. */
  .frow { display:flex; gap:10px; padding:8px 0; border-bottom:1px solid #E5E8EE; position:relative; }
  ...
  .frow.rdrow { background:#fff; margin:0 -8px; padding:8px; border-radius:8px;
    border-inline-start:2px solid #ECEEF2; }
  ...
  .flab { font-size:14px; font-weight:450; color:#656B76; letter-spacing:0; line-height:1.5; }
  .fval { font-size:14px; font-weight:450; color:#14161A; line-height:1.5; min-height:28px;
    margin-top:0; }
  .fval-a { font-size:14px; font-weight:450; color:#33373E; }
  .fval-m { font-size:14px; font-weight:450; color:#656B76; margin-top:0; min-height:28px; }
  .sig { font-size:12px; color:#656B76; margin-top:3px; font-weight:450; }
  .quote { font-size:12px; color:#656B76; margin-top:4px; line-height:1.7; }
  .ferr { font-size: 12px; color: #8E2A27; font-weight: 600; margin-top: 6px; line-height: 1.7; }
  .pen { position: absolute; inset-inline-end: 0; top: 10px; border: none; background: transparent; color: #656B76; cursor: pointer; font-size: 14px; width: 34px; height: 34px; border-radius: 8px; opacity: 0; font-family: inherit; }
  .frow:hover .pen, .frow:focus-within .pen { opacity: 1; }
  .pen[disabled] { cursor: not-allowed; color: #D8DCE3; }
  ...
  .add { font-size: 12px; font-weight: 600; color: #2563EB; background: transparent; border: 1px dashed #C4E8E5; border-radius: 999px; padding: 6px 13px; cursor: pointer; margin-top: 6px; font-family: inherit; display: inline-block; text-decoration: none; }
  .plgnd { display: flex; gap: 14px; flex-wrap: wrap; font-size: 12px; color: #656B76; font-weight: 600; margin: 8px 0 4px; padding-bottom: 10px; border-bottom: 1px solid #E5E8EE; }
  .plgnd .i { display: inline-flex; gap: 6px; align-items: center; }
  .crec :focus-visible { outline: 2px solid #2563EB; outline-offset: 2px; }
  @media (prefers-reduced-motion: no-preference) {
...
<div class="app">
  <aside>
    ...
    <button class="navsearch" onclick="palOpenBox()" aria-label="الانتقال السريع">
      <svg width="15" height="15" style="flex:none;color:#656B76"><use href="#i-search"/></svg>
...
/**
 * Slide a tab strip's indicator under its active tab.
 *
 * transitions.dev's «Tabs sliding». Shared by the door tab strip and the reports tabs so the two
 * cannot drift. Only transform and width are animated — both composite, so this never enters the
 * paint path on a 512MB shared-CPU box that is also serving the webhook.
 *
 * The first paint sets the position with transition suppressed: there is no previous tab for the
 * indicator to travel from, and animating in from the inline-start edge reads as a glitch.
 */
function moveInd(strip) {
  ...
  // Logical inline offset: in RTL the strip's inline-start is its RIGHT edge, so measuring with
  // physical offsetLeft and translating by it would send the indicator the wrong way.
  const sb = strip.getBoundingClientRect(), ob = on.getBoundingClientRect();
  const rtl = getComputedStyle(strip).direction === "rtl";
  const off = rtl ? (sb.right - ob.right) : (ob.left - sb.left);
  ind.style.width = ob.width + "px";
  ind.style.transform = "translateX(" + (rtl ? -off : off) + "px)";
  if (first) requestAnimationFrame(function () { ind.classList.remove("noanim"); });
}
window.moveInd = moveInd;
...
function contactRowsHtml(rows, win) {
  ...
  rows.forEach((r) => {
    ...
    h += '<div class="trow" onclick="location.hash=\\'customer/' + esc(c.phone) + '\\'">' +
      ...
      '<div class="lastm">' + (ci && ci.next_action ? '<span style="color:#2563EB;font-weight:600;">← ' + esc(ci.next_action) + "</span>" : esc(last ? last.text : "—")) + "</div>" +
      '<div class="tm">' + (last ? fmtT(last.ts) : "") + "</div>" +
      '<div style="text-align:left;font-size:12px;color:#2563EB;font-weight:500;" onclick="event.stopPropagation();openConvo(\\'' + esc(c.phone) + '\\')">المحادثة ←</div></div>';
  });
  return h;
}
...
function renderConvo() {
  ...
  el.innerHTML = '<div class="backdrop" onclick="closeConvo()"></div>' +
    ...
    '<div style="flex:1;min-width:0;"><div style="font-size:14px;font-weight:600;color:#14161A;">' + esc(nm) + '</div>' +
    '<div style="font-size:12px;color:#656B76;direction:ltr;text-align:right;">+' + esc(c.phone) + "</div></div>" +
    '<button onclick="closeConvo()" style="font-family:inherit;flex:none;font-size:18px;font-weight:600;color:#656B76;background:#E5E8EE;border:none;border-radius:9px;width:32px;height:32px;cursor:pointer;">×</button></div>' +
    '<div style="padding:9px 16px;border-bottom:1px solid #E5E8EE;display:flex;gap:5px;flex-wrap:wrap;">' + chipRow(c) + " " + interestChips(c) + "</div>" +
...
function testToggleChip(nTest) {
  ...
  return '<button class="btn" style="padding:5px 12px;font-size:12px;border-radius:999px;' +
    (showTest ? 'color:#14161A;background:#ECEEF2;border:1px solid #ECEEF2;' : 'color:#656B76;background:#fff;border:1px dashed #A2A9B4;') +
...
function vKmon(d) {
  ...
  if (campTab === "real" && nReal >= 1 && nReal <= 2 && nTest) {
    h += '<div class="sparse rise">' + ic("eye", 16, "#2563EB") +
  ...
  if (!campaigns.length) {
    h += '<div class="empty" style="padding:60px 20px;"><div class="ic"><span></span></div><div class="t">لا حملات بعد</div><div class="s">أطلق أول حملة من <a href="#aimkt" style="color:#2563EB;font-weight:600;">إنشاء حملة</a> — كل إطلاق يظهر هنا بلوحته وأرقامه الحية.</div></div>';
    return h;
  }
  ...
  h += '<div class="ttoolbar"><span style="position:relative;display:inline-flex;align-items:center;flex:1;min-width:220px;max-width:380px;">' +
    '<span style="position:absolute;inset-inline-start:14px;color:#656B76;display:flex;">' + ic("search", 18) + "</span>" +
    ...
    '<select onchange="setCampSort(this)" class="inp" style="height:46px;border-radius:999px;font-weight:600;color:#14161A;">' +
  ...
  h += '<div style="overflow-x:auto;" class="ms-scroll"><div style="min-width:900px;">' +
    '<div style="display:grid;grid-template-columns:2fr 1.15fr .95fr .7fr .7fr .7fr 1.15fr 44px;gap:12px;padding:14px 22px;background:#EFF1F5;border-bottom:1px solid #ECEEF2;font-size:12px;font-weight:600;color:#656B76;">' +
  ...
  withSt.forEach(({ c, st }, i) => {
    ...
    const stChip = isTest
      ? '<span class="chip c-warn"><span style="width:6px;height:6px;border-radius:999px;background:#7A5600;"></span>تجريبية</span>'
      ...
      : (st.replied ? '<span class="chip c-ok"><span style="width:6px;height:6px;border-radius:999px;background:#12633F;"></span>فيها ردود</span>'
        : '<span class="chip c-blue"><span style="width:6px;height:6px;border-radius:999px;background:#1E5FCC;"></span>بلا ردود بعد</span>');
    h += '<div class="trow km" onclick="location.hash=\\'kmon/' + c.id + '\\'" style="display:grid;grid-template-columns:2fr 1.15fr .95fr .7fr .7fr .7fr 1.15fr 44px;gap:12px;padding:16px 22px;align-items:center;">' +
      '<div style="display:flex;align-items:center;gap:12px;min-width:0;"><span role="img" aria-label="' + (isTest ? "حملة تجريبية" : "حملة فعلية") + '" title="' + (isTest ? "حملة تجريبية (بيئة الاختبار)" : "حملة فعلية") + '" style="width:9px;height:9px;border-radius:999px;flex:none;background:' + (isTest ? "#D8DCE3" : "#2563EB") + ";box-shadow:0 0 0 3px " + (isTest ? "rgba(208,213,221,.28)" : "rgba(31,122,115,.16)") + ';"></span>' +
      '<div style="min-width:0;"><div style="font-size:14px;font-weight:600;color:#14161A;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' + esc(c.name) + '</div>' +
      '<div style="font-size:12px;color:#656B76;margin-top:3px;">' + fmtD(c.created_at) + "</div></div></div>" +
      '<div style="font-size:12px;color:#33373E;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' + esc(c.product || "—") + "</div>" +
      "<div>" + stChip + "</div>" +
      '<div style="text-align:center;font-size:14px;font-weight:600;color:#14161A;font-variant-numeric:tabular-nums;">' + fmtN(st.targeted) + "</div>" +
      '<div style="text-align:center;font-size:14px;font-weight:600;color:#14161A;font-variant-numeric:tabular-nums;">' + fmtN(pct(st.seen, st.targeted)) + "٪</div>" +
      '<div style="text-align:center;font-size:14px;font-weight:600;color:#14161A;font-variant-numeric:tabular-nums;">' + fmtN(pct(st.replied, st.targeted)) + "٪</div>" +
      '<div style="display:flex;align-items:center;gap:9px;"><div class="prog" style="flex:1;height:6px;background:#ECEEF2;border-radius:999px;overflow:hidden;"><i style="display:block;height:100%;width:' + prog + '%;background:#2563EB;border-radius:999px;"></i></div><span style="font-size:12px;font-weight:600;color:#656B76;flex:none;font-variant-numeric:tabular-nums;">' + fmtN(prog) + "٪</span></div>" +
  ...
  if (!withSt.length) {
    ...
    h += campQ.trim()
      ...
      : (campTab === "real"
        ? '<div style="padding:44px;text-align:center;color:#656B76;font-size:14px;line-height:1.9;">لم تُطلق أي حملة فعلية بعد.<br><span class="lnk" onclick="setCampTab(\\'test\\')" style="color:#2563EB;font-weight:600;cursor:pointer;">' + fmtN(nTest) + ' حملة تجريبية محفوظة</span>' + (nTest ? "" : "") + '</div>'
        : '<div style="padding:44px;text-align:center;color:#656B76;font-size:14px;">لا حملات في هذا التبويب</div>');
  }
  h += "</div></div>";
  ...
  h += '<div class="tfoot"><span>' + ic("clock", 14) + ' الأرقام تُحدَّث لحظيًا من حالات تسليم واتساب. لا تقديرات.</span>' +
    (nOver ? '<span style="color:#7A5600;font-weight:600;">تُعرض أحدث ' + fmtN(LIST_CAP) + " حملة من " + fmtN(withSt.length + nOver) + ". ضيّق بالبحث لرؤية الباقي.</span>" : "") + "</div>";
  h += "</div>";
  return h;
}
...
function vKmonDetail(id, d) {
  ...
  if (!camp) return '<div class="empty"><div class="ic"><span></span></div><div class="t">حملة غير موجودة</div><div class="s"><a href="#kmon" style="color:#2563EB;font-weight:600;">→ كل الحملات</a></div></div>';
  ...
  let h = '<a href="#kmon" style="display:inline-flex;align-items:center;gap:6px;font-size:12px;font-weight:600;color:#33373E;text-decoration:none;margin-bottom:14px;">→ كل الحملات</a>' +
  ...
  h += '<div class="card rise" style="display:flex;gap:26px;flex-wrap:wrap;align-items:center;">' +
    '<div style="flex:1;min-width:240px;"><div style="font-size:12px;color:#656B76;font-weight:450;">حكم الحملة</div>' +
    ...
    "</div></div>";
  const cards = [
    ["جهات الاستهداف", st.targeted, "#1E5FCC"], ["أُرسلت", st.sent, "#1E5FCC"], ["وصلت", st.delivered, "#5B8DEF"],
    ["شوهدت", st.seen, "#5B8DEF"], ["ردّوا", st.replied, "#1E5FCC"], ["جهات مهتمة", st.interested, "#1E9E63"],
  ];
  ...
  if (topCause) moves.push(["أبرز أسباب عدم الإغلاق: " + topCause, "عالِج السبب في رسالة الحملة القادمة لهذه الخدمة", "#1E5FCC", "#EAF1FE"]);
  if (moves.length) {
    h += '<div class="card rise"><div style="display:flex;align-items:center;gap:9px;"><h3 style="margin:0;display:flex;align-items:center;gap:8px;">' + ic("spark", 18, "#2563EB") + "الخطوة التالية لهذه الحملة</h3>" +
      ...
      moves.map((m) => '<div style="background:' + m[3] + ';border:1px solid #ECEEF2;border-radius:13px;padding:14px 16px;">' +
        '<div style="font-size:14px;font-weight:600;color:' + m[2] + ';">' + esc(m[0]) + "</div>" +
        '<div style="font-size:12px;color:#33373E;margin-top:5px;line-height:1.8;">' + esc(m[1]) + "</div></div>").join("") + "</div></div>";
  }
  const filters = [
  ...
  h += '<div class="tblwrap"><div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;padding:12px 16px;border-bottom:1px solid #ECEEF2;background:#fff;">' +
    '<span style="font-size:14px;font-weight:600;color:#14161A;flex:none;">جهات الاستهداف</span>' +
    '<span style="font-size:12px;color:#656B76;flex:none;">' + fmtN(shown.length) + " من " + fmtN(rows.length) + "</span>" +
    '<span style="flex:1;"></span>' +
    (shown.length ? '<button class="btn" style="font-size:12px;border-radius:6px;color:#2563EB;background:#fff;border:1px solid #2563EB;font-weight:500;" onclick="startRetarget()">⟲ إعادة استهداف هذه الفئة (' + fmtN(shown.length) + ")</button>" : "") +
    filters.map((f) => '<button class="btn" style="padding:6px 12px;font-size:12px;border-radius:999px;' +
      (campFilter === f[0] ? 'color:#2563EB;background:#DCE8FC;border:1px solid #5B8DEF;' : 'color:#33373E;background:#fff;border:1px solid #ECEEF2;') +
      '" onclick="setCampFilter(\\'' + f[0] + '\\')">' + f[1] + " (" + fmtN(f[2]) + ")</button>").join("") +
    '<input id="rq" value="' + esc(rQ) + '" oninput="rSearch(this)" placeholder="بحث…" style="font-family:inherit;font-size:12px;border:1px solid #ECEEF2;border-radius:999px;padding:7px 13px;background:#EFF1F5;width:130px;">' +
    "</div>" +
    '<div class="thead"><div>العميل</div><div>الحالة</div><div>الاهتمام والجدية</div><div>آخر رسالة</div><div>الوقت</div><div></div></div>' +
    (shown.length ? contactRowsHtml(shown, cwin) : '<div style="padding:30px;text-align:center;color:#656B76;font-size:12px;">لا نتائج</div>') + "</div>";
  return h;
}
...
var CRM_STAGE = [
  { key: "new",         pos: 1, type: "open",    label: "مستهدَف",       dot: "#A2A9B4", hint: "في قائمتك، ولم تُرسل له رسالة بعد" },
  { key: "contacted",   pos: 2, type: "ongoing", label: "تم التواصل",    dot: "#A2A9B4", hint: "وصلته رسالة ولم يردّ بعد" },
...
function vSegBuilder() {
  ...
  if (segPresets && segPresets.length) {
    h += '<div style="font-size:12px;color:#656B76;margin-bottom:9px;">اختيار الفئة يملأ الشروط أدناه، ويمكنكم تعديلها.</div>' +
      ...
      segPresets.map((p) => {
        ...
        return '<button class="btn" style="display:block;text-align:start;padding:13px 15px;border:1px solid #ECEEF2;background:#fff;border-radius:13px;height:auto;" onclick="segUsePreset(\\'' + p.id + '\\')">' +
          '<div style="display:flex;align-items:center;gap:8px;"><span style="font-size:12px;font-weight:600;color:#14161A;">' + esc(p.label) + '</span>' +
          '<span class="chip ' + (p.matched ? "c-ok" : "c-grey") + '">' + fmtN(p.matched) + "</span></div>" +
          '<div style="font-size:12px;color:#656B76;margin-top:6px;line-height:1.8;">' + esc(p.hint) + "</div>" +
  ...
  h += '<div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:12px;">' +
    '<span style="font-size:12px;font-weight:600;color:#656B76;">المطابقة:</span>' +
    ...
    '<span style="flex:1"></span><span style="font-size:12px;font-weight:600;color:#656B76;">النافذة:</span>' +
    [3, 5, 7, 14].map((d) => chipBtn(fmtN(d) + (d >= 11 ? " يومًا" : " أيام"), segWindow === d, "segSetWindow(" + d + ")")).join("") + "</div>";

  const conds = (segDef && segDef.conditions) || [];
  h += conds.map((c, i) =>
    '<div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;padding:11px 13px;border:1px solid #ECEEF2;border-radius:12px;background:#fff;margin-bottom:8px;">' +
    ...
    '<span style="font-size:12px;color:#656B76;flex:1;min-width:120px;">' + (c.beforeDays ? "قبل أكثر من " + fmtN(c.beforeDays) + (c.beforeDays >= 11 ? " يومًا" : " أيام") : c.withinDays ? "خلال آخر " + fmtN(c.withinDays) + (c.withinDays >= 11 ? " يومًا" : " أيام") : "طوال الوقت") + "</span>" +
  ...
  h += '<button class="btn" style="font-size:12px;color:#2563EB;background:#EAF1FE;border:1px solid #C4E8E5;margin-bottom:14px;" onclick="segAddCond()">+ أضف شرطًا</button>';

  // The result. Every zero explains itself — that distinction is the whole feature.
  if (segBusy) h += '<div style="font-size:12px;color:#656B76;padding:10px 0;">جارٍ الحساب…</div>';
    ...
    const pv = segPreview;
    h += '<div style="background:#EAF1FE;border:1px solid #DCE8FC;border-radius:13px;padding:14px 16px;margin-bottom:12px;">' +
      '<div style="font-size:12px;color:#2563EB;line-height:1.9;">' + esc(pv.describe) + "</div>" +
      '<div style="display:flex;align-items:baseline;gap:8px;margin-top:8px;"><span style="font-size:22px;font-weight:600;color:#2563EB;">' + fmtN(pv.matched) + "</span>" +
      '<span style="font-size:12px;font-weight:600;color:#2563EB;">جهة تطابق الآن</span>' +
      '<span style="font-size:12px;color:#656B76;">العضوية تُحدَّث تلقائيًا</span></div>';
    const notes = [];
    ...
    if (!pv.matched && (pv.tooNew || []).length && pv.requiredDays > pv.oldestContactDays) {
      h += '<div class="sparse" style="border-inline-start-color:#7A5600;">' + ic("clock", 16, "#7A5600") +
...
window.alertBar = (txt, bad) => {
  ...
  el.style.cssText = "position:fixed;bottom:22px;right:290px;z-index:var(--z-modal);background:" + (bad ? "#FBE9E9" : "#E6F4EC") +
    ";color:" + (bad ? "#D9534F" : "#1E9E63") + ";font-weight:600;font-size:14px;border-radius:11px;padding:13px 18px;box-shadow:0 8px 24px rgba(16,38,68,.14);";
  el.textContent = txt;
...
function vAffinityBand(selName, matched) {
  ...
  if (prodFilterOn()) {
    h += '<div style="font-size:12px;color:#2563EB;margin-top:10px;">' + fmtN(matched) + " جهة تطابق الفرز الحالي.</div>";
  }
  return h + "</div>";
}
...
function vAimkt() {
  ...
  if (!retargetCohort) {
    h += '<div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:16px;">' +
      ...
      '<span style="flex:1"></span><span style="font-size:12px;color:#656B76;align-self:center;">السلوك يبني شريحة حيّة من سجل المحادثات</span></div>';
  }
  if (!retargetCohort && audMode === "behaviour") {
    h += vSegBuilder();
  } else
  if (retargetCohort) {
    h += '<div style="display:flex;align-items:center;gap:14px;flex-wrap:wrap;border:1px solid #ECEEF2;border-inline-start:3px solid #7A5600;background:#fff;border-radius:10px;padding:16px 18px;">' +
      '<span style="font-size:22px;">⟲</span><div style="flex:1;min-width:220px;">' +
      '<div style="font-size:14px;font-weight:600;color:#14161A;">إعادة استهداف: ' + esc(retargetCohort.label) + " — " + fmtN(retargetCohort.targets.length) + " جهة</div>" +
      '<div style="font-size:12px;color:#656B76;margin-top:5px;">من حملة «' + esc(retargetCohort.campaign) + '» — القائمة مقفلة على هذه الفئة كما رأيتها في صفحة الحملة.</div></div>' +
      '<button class="btn" style="font-size:12px;color:#656B76;background:#fff;border:1px solid #D8DCE3;" onclick="clearRetarget()">مسح والاختيار يدويًا</button></div>';
    ...
    h += '<div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;flex-wrap:wrap;">' +
      '<input id="eq" value="' + esc(entQ) + '" oninput="entSearch(this)" placeholder="ابحث بالاسم أو الرقم…" style="font-family:inherit;flex:1;min-width:200px;font-size:12px;border:1px solid #ECEEF2;border-radius:10px;padding:9px 13px;background:#EFF1F5;">' +
      '<button class="btn" style="font-size:12px;color:#1A47BE;background:#DCE8FC;" onclick="entAllMatching()">' + (allOn ? "إلغاء تحديد المطابقين" : "تحديد المطابقين (" + fmtN(m.length) + ")") + '</button>' +
      (selN ? '<button class="btn" style="font-size:12px;color:#656B76;background:#fff;border:1px solid #D8DCE3;" onclick="entClear()">مسح الاختيار</button>' : "") + "</div>";
    const shown = pageSlice("aud", m);
    if (m.length > PAGE_SIZE) {
      h += '<div style="display:flex;align-items:center;gap:12px;background:#EFF1F5;border:1px solid #ECEEF2;border-radius:10px;padding:12px 16px;margin-bottom:10px;">' +
        '<span style="font-size:18px;font-weight:600;color:#14161A;font-variant-numeric:tabular-nums;">' + fmtN(m.length) + "</span>" +
        '<span style="font-size:12px;color:#33373E;line-height:1.8;">جهة مطابقة للشرائح الحالية. «تحديد المطابقين» يختارهم <b style="font-weight:500;color:#14161A;">جميعًا</b> — والقائمة أدناه تُستعرض صفحةً صفحة إن أردت مراجعتهم.</span></div>';
    }
    h += '<div style="border:1px solid #ECEEF2;border-radius:12px 12px 0 0;overflow:hidden;max-height:420px;overflow-y:auto;" class="ms-scroll">' +
      shown.map((e) => {
        const on = entSel.has(e.id);
        return '<div onclick="entTog(' + e.id + ')" style="display:flex;align-items:center;gap:12px;padding:10px 14px;border-bottom:1px solid #E5E8EE;cursor:pointer;' + (on ? "background:#EAF1FE;" : "") + '">' +
          '<span style="width:17px;height:17px;flex:none;border-radius:5px;display:flex;align-items:center;justify-content:center;font-size:12px;color:#fff;' + (on ? "background:#1E5FCC;" : "border:1.5px solid #D8DCE3;background:#fff;") + '">' + (on ? "✓" : "") + "</span>" +
          '<span style="flex:1;min-width:0;font-size:14px;font-weight:600;color:#14161A;">' + esc(e.name) + "</span>" +
          prodChips(e) + attrChips(e, 2) +
          '<span style="font-size:12px;color:#656B76;direction:ltr;">+' + esc(e.phone) + "</span></div>";
      }).join("") +
      (m.length ? "" : '<div style="padding:22px;text-align:center;color:#656B76;font-size:12px;">لا نتائج مطابقة</div>') + "</div>" +
      (m.length ? '<div class="tfoot" style="border:1px solid #ECEEF2;border-top:0;border-radius:0 0 12px 12px;">' + pageBar("aud", m.length, "جهة") + "</div>" : "");
  }
  h += "</div>";
    ...
    (tpls.length ? '<div role="radiogroup" aria-label="القالب المعتمد" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:14px;margin-bottom:18px;">' +
      tpls.map((t, i) => {
        ...
        return '<div role="radio" tabindex="0" aria-checked="' + (on ? "true" : "false") +
          '" onclick="tplPick(' + i + ')" onkeydown="tplKey(event,' + i + ')" style="cursor:pointer;border:1.5px solid ' + (on ? "#5B8DEF" : "#ECEEF2") +
          ";background:" + (on ? "#EAF1FE" : "#fff") + ';border-radius:16px;padding:18px;transition:.18s ease;' + (on ? "box-shadow:0 0 0 3px rgba(63,182,176,.12);" : "") + '">' +
          '<div style="display:flex;align-items:center;gap:8px;margin-bottom:5px;">' +
          '<span style="width:15px;height:15px;flex:none;border-radius:50%;border:1.5px solid ' + (on ? "#2563EB" : "#D8DCE3") +
          ";background:" + (on ? "#2563EB" : "#fff") + ';box-shadow:inset 0 0 0 2.5px #fff;"></span>' +
          '<span style="font-size:12px;font-weight:600;color:#14161A;">' + esc(t.label) + "</span></div>" +
          '<div style="font-size:12px;color:#656B76;line-height:1.75;">' + esc(t.hint) + "</div>" +
          '<div style="font-size:12px;color:#656B76;margin-top:6px;">لِمن: ' + esc(t.audience) + "</div>" +
          '<div style="display:flex;gap:4px;flex-wrap:wrap;margin-top:8px;">' +
          t.buttons.map((b) => '<span style="font-size:12px;font-weight:600;color:#1E5FCC;background:#DCE8FC;border-radius:999px;padding:4px 12px;">' + esc(b) + "</span>").join("") +
          "</div></div>";
      }).join("") + "</div>" : "") +
    '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:18px;align-items:start;">' +
    '<div><div style="display:flex;align-items:center;gap:10px;margin-bottom:8px;flex-wrap:wrap;"><span style="font-size:12px;color:#656B76;font-weight:600;">نص الرسالة</span>' +
    '<span style="flex:1"></span><button id="cmpbtn" class="btn btn-ghost" style="font-size:12px;padding:7px 13px;display:inline-flex;align-items:center;gap:6px;" onclick="composeMsg()">' + ic("spark", 15, "#2563EB") + "اكتبها بالذكاء الاصطناعي</button></div>" +
    '<textarea oninput="campMsgSet(this)" rows="6" style="font-family:inherit;width:100%;font-size:12px;color:#14161A;border:1.5px solid #ECEEF2;border-radius:12px;padding:13px;line-height:2;resize:vertical;">' + esc(campMsg) + "</textarea>" +
    "</div>" +
    '<div><div style="font-size:12px;color:#656B76;font-weight:600;margin-bottom:8px;">معاينة واتساب — رسالة واحدة بأزرار، والملف يُرسَل عند طلبه</div>' +
    '<div class="wa-prev">' +
...
function mdRender(md) {
  return md.split("\\n").map((raw) => {
    const l = raw.trim();
    if (!l) return "";
    if (l.startsWith("# ")) return '<div style="font-size:14px;font-weight:600;color:#14161A;margin:4px 0 10px;">' + esc(l.slice(2)) + "</div>";
    if (l.startsWith("## ")) return '<div style="font-size:12px;font-weight:600;color:#2563EB;margin:14px 0 6px;">' + esc(l.slice(3)) + "</div>";
    if (l.startsWith("- ") || l.startsWith("* ")) return '<div style="display:flex;gap:8px;padding:2px 0;"><span style="width:5px;height:5px;flex:none;margin-top:9px;border-radius:999px;background:#5B8DEF;"></span><span style="font-size:12px;color:#33373E;line-height:1.9;">' + esc(l.slice(2)) + "</span></div>";
    return '<div style="font-size:12px;color:#33373E;line-height:1.9;">' + esc(l) + "</div>";
  }).join("");
}
...
function uploadZone(scopedProduct) {
  return '<div onclick="kbPick()" style="border:1.5px dashed #D8DCE3;background:#EFF1F5;border-radius:14px;padding:26px 20px;text-align:center;cursor:pointer;">' +
    '<div style="width:44px;height:44px;margin:0 auto 12px;border-radius:12px;background:#DCE8FC;display:flex;align-items:center;justify-content:center;"><span style="width:15px;height:15px;border:2.5px solid #1E5FCC;border-radius:4px;"></span></div>' +
    '<div style="font-size:14px;font-weight:600;color:#14161A;">' + (scopedProduct ? "ارفع ملف الخدمة — PDF أو Word أو PowerPoint" : "أضف خدمة مع ملفها — PDF أو Word أو PowerPoint") + "</div>" +
    '<div style="font-size:12px;color:#656B76;margin-top:7px;line-height:1.9;">الملفات الرسمية المعتمدة فقط · محرك التحليل: Firecrawl AnyDoc · يُحفظ Markdown في Product Hub' + (scopedProduct ? "<br>يُضاف تحت هذه الخدمة ويقرأه المساعد فورًا" : "<br>يُستخرج اسم الخدمة من الملف تلقائيًا") + "</div></div>" +
...
function vKb() {
  ...
  if (skill) {
    ...
    h += '<div class="crmbar rise" style="background:linear-gradient(120deg,#EAF1FE,#fff 55%);border-color:#CFE9E6;">' +
      '<span style="width:34px;height:34px;border-radius:9px;background:linear-gradient(135deg,#5B8DEF,#2563EB);display:flex;align-items:center;justify-content:center;color:#fff;flex:none;">' + ic("spark", 18) + "</span>" +
      '<span style="flex:1;min-width:220px;font-size:12px;color:#33373E;line-height:1.8;">' +
      '<b style="color:#14161A;font-weight:600;">مهارة إنشاء العروض بالذكاء الاصطناعي</b> — حمّلها وأعطها لمساعد ذكاء اصطناعي مع الملف التعريفي للخدمة، فيُنتج لك عرضًا رسميًا (PDF) ترفعه في صفحة كل خدمة.' +
      ' <span style="direction:ltr;color:#656B76;font-size:12px;">' + esc(skill.filename) + "</span></span>" +
  ...
  h += '<div class="tblwrap crmflat kbflat rise"><div style="overflow-x:auto;" class="ms-scroll"><div class="crmgrid">' +
    '<div class="kbrow thead-wide" style="padding:8px 20px 8px 12px;background:#fff;border-bottom:1px solid #ECEEF2;font-size:12px;font-weight:500;color:#656B76;cursor:default;">' +
  ...
  h += rows.map((x) =>
    ...
    '<span class="st"><span class="d" style="background:' + (x.r.hub ? "#12633F" : "#D8DCE3") + ';"></span>' +
      (x.r.hub ? "معتمدة" : "لا معرفة بعد") + "</span>" +
    '<span class="st">' + (x.asset
      ? '<span class="d" style="background:#2563EB;"></span><span class="fn">' + esc(clip(x.asset.filename, 30)) + "</span>"
      : '<span style="color:#656B76;">—</span>') + "</span>" +
    '<span class="fig">' + (x.camps ? fmtN(x.camps) : '<span style="color:#656B76;">—</span>') + "</span>" +
    '<span class="fig">' + (x.warm ? fmtN(x.warm) : '<span style="color:#656B76;">—</span>') + "</span>" +
...
function vKbProduct(name) {
  ...
  if (!r) return '<div class="empty"><div class="ic"><span></span></div><div class="t">الخدمة غير موجودة</div><div class="s"><a href="#kb" style="color:#2563EB;font-weight:600;">→ كل الخدمات</a></div></div>';
  ...
  let h = '<a href="#kb" style="display:inline-flex;align-items:center;gap:6px;font-size:12px;font-weight:600;color:#33373E;text-decoration:none;margin-bottom:14px;">→ كل الخدمات</a>';
  ...
  h += '<div class="card rise" style="display:flex;gap:22px;align-items:center;flex-wrap:wrap;padding:26px 24px;">' +
    '<div style="width:44px;height:44px;flex:none;border-radius:10px;background:#E5E8EE;display:flex;align-items:center;justify-content:center;color:#33373E;font-weight:500;font-size:18px;">' + esc(name.trim().charAt(0)) + "</div>" +
    '<div style="flex:1;min-width:220px;">' +
    '<h1 style="margin:0;font-size:18px;font-weight:600;color:#14161A;letter-spacing:0;">' + esc(name) + "</h1>" +
    ...
    (r.hub && r.hub.source_filename ? '<span style="font-size:12px;color:#656B76;direction:ltr;">' + esc(r.hub.source_filename) + "</span>" : "") +
    "</div></div>" +
  ...
  const cells = [
    ["حملات الخدمة", prodCamps.length, "#14161A"],
    ["محققة", (wlProd && wlProd.won) || 0, "#12633F"],
    ["غير مكتسبة", (wlProd && wlProd.lost) || 0, "#8E2A27"],
    ["قيد التفاوض", (wlProd && wlProd.active) || 0, "#1E5FCC"],
  ];
  h += '<div class="card rise" style="padding:0;overflow:hidden;">' +
    '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));">' +
    cells.map((c, i) => '<div style="padding:20px 22px;' + (i ? "border-inline-start:1px solid #ECEEF2;" : "") + '">' +
      '<div style="font-size:12px;color:#656B76;font-weight:600;">' + c[0] + "</div>" +
    ...
    (prodCamps.length || prodCauses.length
      ? '<div style="border-top:1px solid #ECEEF2;background:#EFF1F5;padding:14px 22px;display:flex;gap:10px;flex-wrap:wrap;align-items:center;">' +
  ...
  const fileRow = (title, sub, chip, btnLabel, onclick) =>
    '<div style="display:flex;align-items:center;gap:14px;padding:18px 22px;border-bottom:1px solid #E5E8EE;flex-wrap:wrap;">' +
    '<div style="width:40px;height:40px;flex:none;border-radius:11px;background:#E5E8EE;display:flex;align-items:center;justify-content:center;color:#33373E;">' + ic("doc", 19) + "</div>" +
    '<div style="flex:1;min-width:200px;"><div style="font-size:14px;font-weight:600;color:#14161A;">' + title + "</div>" +
    '<div style="font-size:12px;color:#656B76;margin-top:4px;line-height:1.8;">' + sub + "</div></div>" +
  ...
  h += '<div class="card" style="padding:0;overflow:hidden;"><div style="padding:18px 22px 0;"><h3 style="margin:0 0 4px;">ملفات الخدمة</h3>' +
    '<div style="font-size:12px;color:#656B76;margin-bottom:14px;">ما يرسله المساعد للعميل، وما يقرأه ليبيع</div></div>' +
  ...
  if (r.hub) {
    h += '<div class="card"><div style="display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap;margin-bottom:8px;">' +
      ...
      '<div style="border-top:1px solid #E5E8EE;padding-top:12px;">' + mdRender(r.hub.md) + "</div></div>";
  }
  if (seedP) {
    h += '<div class="card"><h3>المعرفة الأساسية المدمجة</h3><div style="border:1px solid #ECEEF2;border-radius:12px;overflow:hidden;">' +
  ...
  }
  {
    ...
    h += '<div class="card"><h3>جاهزية الخدمة <span class="meta">' + fmtN(done) + " من " + fmtN(rows.length) + " مكتملة</span></h3>" +
      ...
      rows.map((r) => '<div class="kbrow"><span class="dt" style="background:' + (r[2] ? "#2e9e6b" : "#D8DCE3") + ';"></span>' +
...
function manualRowsHtml() {
  const F = (i, k, ph, w) => '<input class="inp" data-i="' + i + '" data-k="' + k + '" value="' + esc(manualRows[i][k]) + '" oninput="entRowSet(this)" placeholder="' + ph + '" style="flex:' + w + ';min-width:0;font-size:12px;padding:10px 13px;">';
...
window.entFileUpload = async (input) => {
  ...
  try {
    ...
    msg += '<div style="font-size:12px;color:#656B76;margin-top:8px;line-height:1.9;">الأعمدة المكتشفة — الاسم: <b>' + esc(d.columns.name) + '</b> · الجوال: <b>' + esc(d.columns.phone) + "</b>" +
      ((d.columns.tags || []).length ? ' · وسوم: <b style="color:#1E5FCC;">' + d.columns.tags.map(esc).join("، ") + "</b>" : "") +
    ...
    if (d.tagsRefused) {
      msg += '<div style="font-size:12px;color:#7A5600;margin-top:6px;line-height:1.9;">عمود الوسوم يحمل ' +
    ...
    if (d.skippedRows && d.skippedRows.length) {
      msg += '<div style="font-size:12px;color:#D9534F;margin-top:4px;line-height:1.9;">' +
...
function vMorningList() {
  ...
  const GROUPS = [
    ...
    ["interested", "مهتم بلا موعد", "#1E5FCC", "أبدوا اهتمامًا ولم يُحدَّد وقت بعد"],
    ["later", "مؤجل", "#656B76", "طلبوا التأجيل"],
  ...
  let h = '<div class="crmbar rise">' + GROUPS.map(([k, label, ink]) =>
    ...
    '<span style="font-size:12px;color:#656B76;">' + fmtN(cs.length) + " جهة في السجل" +
  ...
  for (const [key, label, ink, hint] of GROUPS) {
    ...
    // Group header on the same #EFF1F5 bar as #customers' group view — one grouped-list idiom in
    ...
    h += '<div class="tblwrap crmflat rise" style="margin-bottom:14px;">' +
      '<div style="display:flex;align-items:center;gap:9px;padding:9px 20px 9px 12px;border-bottom:1px solid #ECEEF2;background:#EFF1F5;">' +
      '<span style="width:7px;height:7px;border-radius:999px;flex:none;background:' + ink + ';"></span>' +
      '<span style="font-size:14px;font-weight:500;color:#14161A;">' + label + "</span>" +
      '<span class="cntpill">' + fmtN(rows.length) + "</span>" +
      '<span style="font-size:12px;color:#656B76;">' + hint + "</span></div>";
    if (!rows.length) {
      h += '<div style="padding:16px 20px;font-size:12px;color:#656B76;">' +
    ...
    h += shown.map((c) => {
      ...
      const when = key === "scheduled" && c.scheduledSaid
        ? '<span style="font-size:12px;font-weight:500;color:#14161A;">«' + esc(c.scheduledSaid) + "»</span>" +
          (conf ? '<span style="font-size:12px;color:#12633F;"> · مؤكَّد ' + conf + "</span>"
            : ap ? '<span style="font-size:12px;color:#656B76;"> · قراءتنا ' + esc(fmtT(ap.at)) + " · لم تُؤكَّد بعد</span>"
                 : '<span style="font-size:12px;color:#656B76;"> · لم نقرأ تاريخًا</span>')
        ...
        : conf
          ...
          : c.outcomeReason
            ? '<span style="font-size:12px;color:#656B76;">«' + esc(String(c.outcomeReason).slice(0, 80)) + "»</span>"
            : '<span style="font-size:12px;color:#656B76;">—</span>';
  ...
  if (unsorted.length) {
    h += '<div style="font-size:12px;color:#656B76;padding:4px 2px 8px;">' +
...
function ratesStrip(agg) {
  ...
  return '<div class="card rise" style="margin:0;padding-bottom:2px;">' +
    ...
    '<span style="font-size:12px;color:#656B76;">كل نسبة ومقامها معها</span></div>' +
    rows.map((r) => '<div style="display:flex;align-items:center;gap:12px;padding:11px 0;border-top:1px solid #ECEEF2;">' +
      '<span style="flex:0 0 106px;font-size:12px;color:#14161A;">' + r[0] + "</span>" +
      '<span style="flex:0 0 52px;font-size:16px;font-weight:600;color:' + (r[1] === null ? "#A2A9B4" : "#14161A") +
        ';font-variant-numeric:tabular-nums;">' + (r[1] === null ? "—" : fmtN(r[1]) + '<span style="font-size:12px;color:#656B76;font-weight:450;">٪</span>') + "</span>" +
      '<span style="flex:0 0 88px;font-size:12px;color:#656B76;">' + r[2] + "</span>" +
      '<span style="flex:1;min-width:60px;height:6px;background:#E5E8EE;border-radius:999px;overflow:hidden;">' +
        (r[1] === null ? "" : '<i style="display:block;height:100%;width:' + Math.min(100, r[1]) + '%;background:#2563EB;border-radius:999px;"></i>') + "</span></div>").join("") + "</div>";
}
function chartCard(title, sub, inner) {
  return '<div class="card" style="margin:0;"><div style="display:flex;align-items:baseline;justify-content:space-between;gap:8px;"><h3 style="margin:0;">' + title + '</h3><span style="font-size:12px;color:#656B76;">' + sub + "</span></div>" + inner + "</div>";
}
function hbarRows(rows, color) {
  ...
  return '<div style="margin-top:12px;display:flex;flex-direction:column;gap:9px;">' + rows.map((r) =>
    '<div><div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:4px;"><span style="font-weight:600;color:#14161A;">' + esc(String(r[0])) + '</span><span style="font-weight:600;color:#14161A;">' + fmtN(r[1]) + "</span></div>" +
    '<div style="height:8px;background:#ECEEF2;border-radius:999px;overflow:hidden;"><i style="display:block;height:100%;border-radius:999px;width:' + Math.round(r[1] / mx * 100) + "%;background:" + (r[2] || color) + ';"></i></div></div>').join("") + "</div>";
}
...
const DEAL_META = {
  ...
  active:  ["نشطة",       "#1E5FCC", "#EAF1FE", "الحوار مستمر طبيعيًا"],
};
// ---------------------------------------------------------------------------
...
function vActionQueue(cs, notifyNumber, nTest) {
  ...
  stalled.slice(0, 2).forEach((c) => { const ins = insCache[c.phone] || {};
    items.push({ c, dot: "#1E5FCC", why: "صفقة متوقفة" + (ins.loss_cause ? " · " + ins.loss_cause : ""),
  ...
  let h = '<div class="card rise' + (anyAct ? "" : " noact") + '" style="margin-bottom:18px;padding-bottom:0;overflow:hidden;">' +
    ...
    '<div style="font-size:12px;color:#656B76;margin-top:4px;">قائمة الصباح الوحيدة — مرتَّبة بالأكثر إلحاحًا، وكل سطر يذكر سببه.</div></div>' +
  ...
  if (!items.length) {
    h += '<div style="padding:26px 2px 30px;font-size:12px;color:#656B76;line-height:1.9;border-top:1px solid #ECEEF2;">' +
    ...
    h += items.map(function (it) {
      ...
      return '<div class="aq" onclick="location.hash=&quot;' + it.href + '&quot;">' +
        ...
        (c ? '<span class="aqav">' + esc((c.waName || "؟").trim().charAt(0)) + "</span>"
           : '<span class="aqav aqic">' + ic(it.icon, 16, "#656B76") + "</span>") +
  ...
  if (notifyNumber) {
    h += '<div style="display:flex;align-items:center;gap:8px;padding:11px 2px;border-top:1px solid #ECEEF2;font-size:12px;color:#656B76;">' +
      ic("send", 14, "#A2A9B4") + '<span>تنبيهات «عميل جاد» و«طلب تدخّل» تصل واتساب مدير المنتج</span>' +
      '<b style="color:#33373E;font-weight:500;direction:ltr;">+' + esc(notifyNumber) + "</b></div>";
  }
  return h + "</div>";
}
function vWinLoss() {
  ...
  let h = '<div class="card" style="margin-bottom:18px;">' +
    ...
    '<div style="font-size:12px;color:#656B76;margin-top:6px;line-height:1.8;">يحلّل المساعد كل محادثة بناءً على محتواها الفعلي، ويوضّح سبب التصنيف والأدلة الداعمة.</div>';
  if (!judged && !(t.active || 0)) {
    h += '<div style="font-size:12px;color:#656B76;margin-top:14px;line-height:1.9;">يتعبأ هذا اللوح مع أول محادثة مصنَّفة — وكل محادثة محققة أو غير مكتسبة ستظهر هنا بسبب تصنيفها.</div></div>';
    return h;
  }
  ...
  if (nWin) {
    h += '<div><div style="font-size:12px;font-weight:500;color:#12633F;margin-bottom:9px;">عوامل ساعدت على التقدّم</div>' +
      winloss.win_drivers.map((w) => '<div style="display:flex;align-items:center;gap:9px;padding:9px 0;border-top:1px solid #ECEEF2;"><span style="flex:1;font-size:12px;color:#14161A;line-height:1.8;">' + esc(w.driver) + "</span>" + cnt(w.count) + "</div>").join("") + "</div>";
  }
  if (nLoss) {
    h += '<div><div style="font-size:12px;font-weight:500;color:#8E2A27;margin-bottom:9px;">أسباب التوقّف أو عدم الاكتساب</div>' +
      winloss.loss_causes.map((c) => '<div style="padding:9px 0;border-top:1px solid #ECEEF2;"><div style="display:flex;align-items:center;gap:9px;"><span style="flex:1;font-size:12px;font-weight:500;color:#14161A;">' + esc(c.cause) + "</span>" + (c.products || []).map((pd) => '<span class="chip">' + esc(clip(pd, 22)) + "</span>").join("") + cnt(c.count) + "</div>" +
        (c.example ? '<div style="font-size:12px;color:#656B76;margin-top:4px;line-height:1.8;">« ' + esc(c.example) + ' »</div>' : "") + "</div>").join("") + "</div>";
  }
  h += "</div>";
  if (!nWin || !nLoss) {
    h += '<div style="font-size:12px;color:#656B76;margin-top:' + (nWin || nLoss ? "14px;padding-top:12px;border-top:1px solid #ECEEF2;" : "10px;") + '">' +
  ...
  if (judgedProducts.length) {
    h += '<div style="margin-top:14px;padding-top:12px;border-top:1px solid #ECEEF2;">' +
      judgedProducts.map((pr) => '<div style="display:flex;align-items:center;gap:10px;padding:6px 0;font-size:12px;">' +
        '<span style="flex:1;min-width:0;color:#14161A;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' + esc(pr.product) + "</span>" +
        '<span style="color:#12633F;">' + fmtN(pr.won) + ' محققة</span><span style="color:#656B76;">·</span>'  +
...
function vSignalBoard(d) {
  ...
  const BAND = { ready: ["#12633F", "#E4F5EC"], serious: ["#2563EB", "#EAF6F5"], watch: ["#7A5600", "#FFF5D6"], cold: ["#656B76", "#E5E8EE"] };
  ...
  const MOVE = { rising: ["▲", "#12633F"], steady: ["■", "#656B76"], cooling: ["▼", "#7A5600"], silent: ["○", "#8E2A27"], none: ["·", "#A2A9B4"] };
  ...
  const silent = s.daysSilent === null || s.daysSilent === undefined
    ? '<span style="color:#656B76;font-weight:500;">—</span>'
  ...
  act.forEach((a) => {
    ...
    bars += '<span title="' + esc(title) + '" style="flex:1;min-width:4px;display:flex;flex-direction:column;justify-content:flex-end;height:' + fmtN2(H) + 'px;">' +
      (inb ? '<span style="display:block;height:' + fmtN2(Math.max(hi, 3)) + 'px;background:#2563EB;border-radius:2px 2px 0 0;"></span>' : "") +
      (outb ? '<span style="display:block;height:' + fmtN2(Math.max(ho, 2)) + 'px;background:#D8DCE3;border-radius:' + (inb ? "0" : "2px 2px 0 0") + ';"></span>' : "") +
      (!inb && !outb ? '<span style="display:block;height:2px;background:#E5E8EE;border-radius:2px;"></span>' : "") + "</span>";
  });
  ...
  const why = (s.factors || []).length
    ? '<details style="margin-top:12px;"><summary style="cursor:pointer;font-size:12px;color:#2563EB;font-weight:600;list-style:none;">لماذا هذه القراءة؟</summary>' +
      '<div style="margin-top:8px;display:flex;flex-direction:column;gap:6px;">' +
      s.factors.map((f) =>
        '<div style="display:flex;gap:9px;align-items:baseline;font-size:12px;line-height:1.8;">' +
        '<span style="font-weight:600;min-width:34px;color:' + (f.points < 0 ? "#8E2A27" : f.points > 0 ? "#12633F" : "#656B76") + ';">' +
        (f.points === 0 ? "·" : (f.points < 0 ? "−" : "+") + fmtN(Math.abs(f.points))) + "</span>" +
        '<span style="color:#14161A;font-weight:600;min-width:96px;">' + esc(f.label) + "</span>" +
        '<span style="color:#33373E;flex:1;min-width:140px;">' + esc(f.evidence) + "</span></div>").join("") +
      "</div></details>"
    : "";
  ...
  const next = hasWords(ins.next_action)
    ? '<div style="margin-top:14px;background:#fff;border:1px solid #DCE8FC;border-inline-start:3px solid #2563EB;border-radius:11px;padding:11px 14px;">' +
      '<div style="font-size:12px;font-weight:600;color:#2563EB;margin-bottom:4px;">اقتراح المساعد للخطوة التالية</div>' +
      '<div style="font-size:14px;font-weight:600;color:#14161A;line-height:1.85;">' + esc(ins.next_action) + "</div></div>"
    : "";

  return '<div class="card" style="margin:2px 0 14px;">' +
    '<div style="display:flex;align-items:center;gap:8px;">' +
    '<h3 style="margin:0;font-size:14px;font-weight:600;color:#14161A;display:flex;align-items:center;gap:8px;">' + ic("spark", 18, "#2563EB") + "مؤشرات المحادثة</h3>" +
    '<span style="font-size:12px;color:#656B76;">محسوبة من السجل — لا من قراءة نموذج</span></div>' +
    '<div style="display:flex;gap:26px;flex-wrap:wrap;align-items:flex-start;margin-top:14px;">' +
    // 1 — the meter
    '<div style="min-width:200px;flex:1;">' +
    '<div style="font-size:12px;font-weight:600;color:#656B76;letter-spacing:0;">الجدية</div>' +
    ...
    '<span style="font-size:12px;font-weight:600;color:#656B76;letter-spacing:0;">النشاط · ' + esc(arDaysUi(act.length)) + "</span>" +
    '<span style="font-size:12px;color:#656B76;"><span style="display:inline-block;width:8px;height:8px;border-radius:2px;background:#2563EB;margin-inline-end:5px;"></span>رسائله</span>' +
    '<span style="font-size:12px;color:#656B76;"><span style="display:inline-block;width:8px;height:8px;border-radius:2px;background:#D8DCE3;margin-inline-end:5px;"></span>رسائلنا</span></div>' +
    ...
    (total
      ? '<div style="display:flex;align-items:flex-end;gap:3px;margin-top:10px;">' + bars + "</div>" +
        '<div style="display:flex;justify-content:space-between;margin-top:6px;font-size:12px;color:#656B76;">' +
        '<span>' + esc(act.length ? fmtD(act[0].day) : "") + "</span><span>" +
        esc(act.length ? fmtD(act[act.length - 1].day) : "") + "</span></div>"
      : '<div style="font-size:12px;color:#656B76;margin-top:10px;">لا رسائل خلال آخر ' + esc(arDaysUi(act.length)) + ".</div>") +
    "</div>" +
    next + why + "</div>";
}
...
function tlDot(kind) {
  ...
  return { in: "#2563EB", out: "#656B76", camp: "#A2A9B4", file: "#A2A9B4", tag: "#A2A9B4", st: "#A2A9B4", sys: "#D8DCE3" }[kind] || "#D8DCE3";
}
// ---------------------------------------------------------------------------
...
function propEditorHtml(key, val, err) {
  ...
  if (key === "nextStep") {
    ...
    h += '<div style="display:flex;gap:8px;align-items:center;margin-top:6px;flex-wrap:wrap;">' +
      '<label for="propdue" style="font-size:12px;color:#656B76;font-weight:600;">التاريخ (اختياري)</label>' +
...
function propRow(o) {
  ...
  let body = '<div class="flab">' + o.label +
    (o.suffix ? ' <span style="color:#656B76;font-weight:450;">' + o.suffix + "</span>" : "") + "</div>";
  if (open) {
    ...
    if (o.contested) {
      body += '<div class="quote" style="color:#7A5600;">قراءة مختلفة من المساعد: «' + esc(o.contested.value) + "»</div>" +
...
function vAccountPanel(d) {
  ...
  var h = '<div class="card" style="margin:14px 0 0;">' +
    ...
    "</div>" +
    '<div style="font-size:12px;color:#656B76;margin:6px 0 4px;line-height:1.8;">' +
  ...
  if (!e) {
    return h + '<div class="fval-m">لا سجل جهة لهذا الرقم بعد، فلا يستطيع المساعد حفظ ما يقوله العميل عن نظامه. ' +
      '<a href="#customers" style="color:#2563EB;font-weight:600;">→ جهات الاستهداف</a></div></div>';
  }
  ...
  rows.forEach(function (k) {
    ...
    if (!v) body += '<div class="fval-m">لم يُعرف بعد — المساعد مصرّح له بالسؤال عنه.</div>';
      ...
      (v.contested
        ? '<div class="quote" style="color:#7A5600;">قراءة مختلفة من المساعد: «' + esc(v.contested.value) + "»</div>"
        : "");
...
function vFactsPanel(d) {
  ...
  let h = '<div class="card" style="margin:0;">' +
    ...
    '<div style="font-size:12px;color:#656B76;margin:6px 0 4px;line-height:1.8;">ما تكتبه هنا لا يستطيع المساعد تغييره.</div>' +
  ...
  const parsed = !ap4
    ...
    : ap4.confirmed
      ? '<div style="display:flex;gap:8px;align-items:flex-start;margin-top:5px;">' + pmSpan("h", "margin-top:5px") +
        '<div style="margin:0;font-size:12px;font-weight:600;color:#14161A;">الموعد: ' + esc(fmtDay(ap4.at)) +
...
function vCustomer(ph) {
  ...
  h += '<div class="card" style="display:flex;gap:18px;align-items:stretch;flex-wrap:wrap;">' +
    ...
    '<div style="font-size:18px;font-weight:600;color:#14161A;">' + esc(nm) + "</div>" +
    (function () {
      ...
      return '<span class="chip" style="font-size:12px;padding:4px 10px;" title="' + esc(sg.hint) + '">' +
        ...
        sg.label + "</span>" +
        (since ? '<span style="font-size:12px;color:#656B76;">منذ ' + fmtAgo(Date.now() - since) + "</span>" : "");
    })() +
    ...
    "</div>" +
    '<div style="font-size:12px;color:#656B76;margin-top:8px;direction:ltr;text-align:right;">+' + esc(c.phone) + "</div>" +
    // «أول ظهور» deleted — it never changed a decision. Last activity stays: it tells you whether
    // this person is warm right now.
    '<div style="font-size:12px;color:#656B76;margin-top:4px;">آخر نشاط: ' + fmtT(c.lastEventAt) + "</div>" +
    ...
    ((d.campaigns || []).length ? (function () {
      ...
      return '<div style="margin-top:10px;font-size:12px;color:#33373E;line-height:1.7;">' +
        (scoped ? "مقصور على حملة: " : "بدأت هذه المحادثة من: ") +
        '<a href="#customer/' + esc(c.phone) + "/" + first.id + '" style="color:#2563EB;font-weight:600;text-decoration:none;">' +
        esc(String(first.name).slice(0, 40)) + "</a>" +
        (known ? ' <span style="color:#656B76;">· ' + esc(when) + "</span>"
               : ' <span style="color:#656B76;">· وقت الإطلاق غير مقروء، فلا تُنسب أرقام لهذه الحملة</span>') +
        (others > 0 ? ' <span style="color:#656B76;">· وسبقتها ' + fmtN(others) + " حملة</span>" : "") +
        (testN > 0 && showTest ? ' <span style="color:#656B76;">(+' + fmtN(testN) + " تجريبية)</span>" : "") +
        (scoped ? ' <a href="#customer/' + esc(c.phone) + '" style="color:#656B76;text-decoration:underline;">عرض كل التاريخ</a>' : "") +
        (!real.length && cps.length ? '<div style="margin-top:5px;font-size:12px;color:#656B76;">لا حملة فعلية بعد — الحملات التجريبية مخفية.</div>' : "") +
        "</div>";
    })() : "") +
    "</div></div>" +
    ...
    (function () {
      ...
      return turn ? '<div style="font-size:12px;color:#656B76;margin-top:6px;">' + turn + "</div>" : "";
    })() +
    "</div></div>";
  ...
  h += (function () {
    const OUT = {
      ...
      closed: ["مغلق", "#656B76", "#E5E8EE"],
    };
    const o = OUT[c.outcome] || null;
    const ink = o ? o[1] : "#656B76", bg = o ? o[2] : "#EFF1F5";
    const row = (label, body) =>
      '<div style="display:flex;gap:10px;align-items:baseline;flex-wrap:wrap;margin-top:9px;">' +
      '<span style="font-size:12px;font-weight:600;letter-spacing:0;color:#656B76;min-width:92px;">' + label + "</span>" +
      '<span style="flex:1;min-width:180px;">' + body + "</span></div>";

    // 1 — OUTCOME
    let outBody = '<span style="font-size:14px;font-weight:600;color:' + ink + ';">' + (o ? o[0] : "لم يُفرز بعد") + "</span>";
    if (c.outcome === "scheduled" && c.scheduledSaid) {
      ...
      const ap = appt(c);
      outBody += '<div style="font-size:14px;font-weight:600;color:#14161A;margin-top:3px;">قال العميل: «' + esc(c.scheduledSaid) + "»</div>" +
        '<div style="font-size:12px;color:' + (ap && ap.confirmed ? "#12633F;font-weight:600;" : "#656B76;") + '">' +
      ...
      outBody += '<div style="font-size:12px;color:#33373E;margin-top:3px;">لأنه قال: «' + esc(String(c.outcomeEvidence).slice(0, 130)) + "»</div>";
    }
    ...
    if (tagList.length) {
      ...
      interestBody = '<span class="chip c-read ' + rd[0] + '"><span class="rd">قراءة</span>' + rd[1] + "</span>" +
        '<div style="font-size:12px;color:#656B76;margin-top:3px;">لا وسم اهتمام مؤكد بعد — هذه قراءة المساعد من نص المحادثة.</div>';
    } else {
      interestBody = '<span style="color:#656B76;font-size:12px;">لم يُسجَّل اهتمام بعد.</span>';
    }
    ...
    if (ins.stage) {
      ...
      stageBody = '<span class="chip ' + (justified ? "c-teal" : "c-grey") + '">' + esc(ins.stage) +
        "</span>" +
        (justified
          ? '<div style="font-size:12px;color:#33373E;margin-top:3px;line-height:1.9;">' + esc(ins.stage_reason) + "</div>"
          : '<div style="font-size:12px;color:#656B76;margin-top:3px;">قراءة المساعد — بلا اقتباس يسندها بعد. ' +
            '<a href="javascript:void(0)" onclick="refreshInsights()" style="color:#2563EB;font-weight:600;">حدّث القراءة</a></div>');
    } else {
      stageBody = '<span style="color:#656B76;font-size:12px;">لم تتحدد المرحلة بعد.</span>';
    }

    /* neutral surface: the tint was the last coloured band on this record (DESIGN.md §2) */
    return '<div style="background:#fff;border:1px solid #ECEEF2;border-inline-start:3px solid ' + ink +
  ...
  h += '<div style="display:flex;gap:16px;flex-wrap:wrap;margin:2px 0 16px;align-items:center;background:#fff;border:1px solid #ECEEF2;border-radius:10px;padding:10px 14px;">' +
    ...
    '<div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;">' +
    '<span style="font-size:12px;color:#656B76;white-space:nowrap;">سجّل النتيجة الفعلية</span>' +
    [["meeting_booked", "اجتماع محجوز", "#12633F"], ["quote_sent", "عرض مُرسَل", "#1E5FCC"], ["postponed", "مؤجل", "#7A5600"], ["not_a_fit", "غير مناسب", "#656B76"]]
  ...
  h += '<div class="card rise" style="margin:0;">' +
    ...
    '<div style="display:flex;align-items:center;gap:8px;">' + pmSpan("a", "margin:0") + '<h3 style="margin:0;color:#14161A;font-size:14px;font-weight:600;display:flex;align-items:center;gap:8px;">' + ic("spark", 19, "#2563EB") + "فهم المساعد</h3></div>" +
    '<div style="font-size:12px;color:#656B76;margin-top:6px;">كل ما في هذه البطاقة قراءة، لا حقيقة مسجّلة.</div>';
  if (ins.learning) {
    ...
    h += '<div style="font-size:14px;color:#33373E;line-height:2;margin-top:12px;">' + esc(ins.summary) + "</div>" +
      '<div style="font-size:12px;color:#656B76;margin-top:12px;line-height:1.9;">كل رسالة جديدة تجعل القراءة أدق — كما في مرحلة «Learning…».</div>';
  } else {
    h += '<div style="background:#fff;border:1px solid #ECEEF2;border-radius:13px;padding:15px 16px;margin-top:14px;">' +
      '<div style="font-size:12px;font-weight:600;color:#2563EB;margin-bottom:7px;">الخلاصة</div>' +
      '<div style="font-size:14px;font-weight:600;color:#14161A;line-height:1.95;">' + esc(ins.summary || "") + "</div></div>";
    ...
    if ((ins.signals || []).length) h += '<div style="margin-top:12px;"><div style="font-size:12px;font-weight:600;color:#656B76;margin-bottom:6px;">إشارات الشراء</div>' + ins.signals.map((sg) => '<div style="font-size:12px;color:#14161A;line-height:1.9;">« ' + esc(sg) + ' »</div>').join("") + "</div>";
    if ((ins.objections || []).length) h += '<div style="margin-top:10px;"><div style="font-size:12px;font-weight:600;color:#656B76;margin-bottom:6px;">اعتراضات</div>' + ins.objections.map((ob) => '<div style="font-size:12px;color:#8a5a2b;line-height:1.9;">· ' + esc(ob) + "</div>").join("") + "</div>";
    ...
    h += '<div style="display:flex;align-items:center;gap:8px;margin-top:12px;">' +
      ...
      (hasWords(ins.evidence) && String(ins.evidence).trim() !== String(c.outcomeEvidence || "").trim()
        ? '<div style="font-size:12px;color:#656B76;margin-top:7px;line-height:1.8;">الدليل: « ' + esc(ins.evidence) + ' »</div>' : "") +
    ...
    h += '<div style="margin-top:14px;background:#fff;border:1px solid #DCE8FC;border-inline-start:3px solid #2563EB;border-radius:11px;padding:13px 15px;">' +
      ...
      '<div style="font-size:12px;font-weight:600;color:#2563EB;margin-bottom:5px;">اقتراح المساعد للخطوة التالية</div>' +
      '<div style="font-size:14px;font-weight:600;color:#14161A;line-height:1.9;">' + esc(ins.next_action || "") + "</div>" +
      (ins.why ? '<div style="font-size:12px;color:#33373E;margin-top:5px;line-height:1.9;">' + esc(ins.why) + "</div>" : "") +
      (ins.best_time ? '<div style="font-size:12px;color:#2563EB;font-weight:600;margin-top:7px;">وقت التواصل: ' + esc(ins.best_time) + "</div>" : "") + "</div>";
  }
  h += "</div>";
  // timeline
  h += '<div class="card" style="margin:0;"><h3 style="margin:0 0 4px;">سجل التفاعل</h3>' +
    '<div style="font-size:12px;color:#656B76;margin-bottom:10px;">كل نقاط التماس — رسائل، حالات تسليم، وسوم، ملفات — الأحدث أولًا</div>' +
    ...
    ((d.timeline || []).length ? d.timeline.map((ev) =>
      ...
      '<span style="position:absolute;inset-inline-start:5px;top:24px;bottom:-11px;width:2px;background:#ECEEF2;"></span>' +
      ...
      '<div style="font-size:12px;color:#14161A;line-height:1.8;margin-top:3px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' + esc(ev.title) + "</div></div></div>").join("")
      : '<div style="padding:20px;text-align:center;color:#656B76;font-size:12px;">لا أحداث بعد</div>') + "</div></div>";
  h += "</div></div>";
  return h;
}
...
function gate(msg) {
  document.getElementById("body").innerHTML = '<div class="gate"><div style="font-size:16px;font-weight:600;">الدخول إلى مسار</div>' +
    '<input id="tok" placeholder="admin token" dir="ltr"><button class="btn btn-teal" onclick="saveTok()">دخول</button>' +
    (msg ? '<div style="color:#D9534F;font-size:12px;margin-top:10px;">' + esc(msg) + "</div>" : "") + "</div>";
}
/**
...
async function gateUnauthorized() {
  ...
  if (isRep) {
    document.getElementById("body").innerHTML =
      ...
      '<div style="color:#33373E;font-size:14px;line-height:1.9;margin-top:8px;max-width:44ch;">' +
