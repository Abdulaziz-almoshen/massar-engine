...
export const CAMPAIGNS_CRM_CSS = `
  ...
  .crmbar { display:flex; align-items:center; gap:10px; flex-wrap:wrap; padding:12px 16px;
    background:#fff; border:1px solid #EFEDF7; border-radius:13px; margin-bottom:14px; }
  .crmbar .hair { width:1px; height:22px; background:#EFEDF7; flex:none; }
  .vtog { display:inline-flex; background:#E9E6F4; border-radius:999px; padding:3px; flex:none; }
  .vtog button { font-family:inherit; font-size:12px; font-weight:600; color:#6B6880; background:transparent;
    border:none; border-radius:999px; padding:7px 15px; cursor:pointer; white-space:nowrap; }
  .vtog button.on { background:#fff; color:#6C5CE7; box-shadow:0 1px 3px rgba(16,24,40,.10); }
  .qpill { font-family:inherit; font-size:12px; font-weight:600; border-radius:999px; padding:7px 13px;
    cursor:pointer; white-space:nowrap; color:#35333F; background:#fff; border:1px solid #EFEDF7; }
  .qpill.on { color:#6C5CE7; background:#DDD6F7; border-color:#8B7BF5; }
  .crmsel { font-family:inherit; height:38px; border:1px solid #EFEDF7; border-radius:999px;
    background:#F0EEF9; color:#16151F; font-size:12px; font-weight:600; padding:0 12px; cursor:pointer; }

  /* ===== selection ===== */
  .selcell { width:40px; flex:none; display:flex; align-items:center; justify-content:center; }
  .selcell input { width:16px; height:16px; accent-color:#6C5CE7; cursor:pointer; }
  ...
  .krow.sel { background:#FFFFFF; border-inline-start: 3px solid #6C5CE7; }

  /* ===== bulk bar ===== */
  .bulkbar { position:fixed; inset-block-end:18px; inset-inline:0; display:flex; justify-content:center;
    z-index:var(--z-toast); pointer-events:none; padding:0 12px; }  /* above alertBar (z-index:99), which occluded this for its full lifetime on the select-page path */
  .bulkbar > div { pointer-events:auto; display:flex; align-items:center; gap:9px; flex-wrap:wrap;
    background:#16151F; color:#fff; border-radius:999px; padding:9px 14px; max-width:92vw;
    box-shadow:0 12px 32px rgba(16,24,40,.28); }
  .bulkbar .cnt { font-size:12px; font-weight:600; background:rgba(255,255,255,.14); border-radius:999px;
    padding:5px 12px; white-space:nowrap; }
  .bulkbar button { font-family:inherit; font-size:12px; font-weight:600; border-radius:999px;
    padding:7px 13px; cursor:pointer; border:1px solid rgba(255,255,255,.22); background:transparent;
    color:#fff; white-space:nowrap; }
  .bulkbar button.pri { background:#8B7BF5; border-color:#8B7BF5; color:#16151F; }
  ...
  .kcol { flex:none; width:290px; background:#F0EEF9; border:1px solid #EFEDF7; border-radius:13px; padding:11px; }
  .kcol.over { border-color:#8B7BF5; background:#EDEAFD; }
  .kcolh { display:flex; align-items:center; gap:8px; margin-bottom:10px; padding:0 4px; }
  .kcolh .lb { font-size:12px; font-weight:600; color:#16151F; }
  .kcard { background:#fff; border:1px solid #EFEDF7; border-radius:10px; padding:12px;
    margin-bottom:9px; cursor:pointer; display:block; text-decoration:none; }
  .kcard:hover { border-color:#6B6880; }
  .kcard:focus-visible { outline:2px solid #6C5CE7; outline-offset:-2px; }
  .kcard .ktitle { display:flex; align-items:center; gap:8px; min-width:0; }
  .kcard .kline { font-size:14px; font-weight:450; color:#35333F; margin-top:6px;
    overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
  .kcard .kfoot { display:flex; flex-wrap:wrap; align-items:baseline; gap:4px 10px;
    border-top:1px solid #EFEDF7; margin-block-start:10px; padding-block-start:8px; }
  .kcard .kfoot .kl { font-size:12px; color:#6B6880; }
  .kcard .kfoot .kv { font-size:14px; font-weight:500; color:#16151F; font-variant-numeric:tabular-nums; }
  .kcard .kfoot .ksep { color:#6B6880; font-size:12px; }
  .kcard:hover { border-color:#8B7BF5; }
  .kcard .nm { font-size:14px; font-weight:500; color:#16151F; overflow:hidden;
    text-overflow:ellipsis; white-space:nowrap; }
  .kdrop { border:1.5px dashed #D6D1E8; border-radius:11px; padding:18px 12px; text-align:center;
    color:#6B6880; font-size:12px; }

  /* ===== detail: spec strip + tabs ===== */
  .spec { display:flex; gap:18px; flex-wrap:wrap; align-items:flex-start; background:#F0EEF9;
    border:1px solid #EFEDF7; border-radius:13px; padding:16px 18px; margin-bottom:16px; }
  ...
  .spec .bub2 { background:#DCF8C6; border-radius:9px; padding:10px 12px; font-size:12px; color:#16151F;
  ...
  .spec .facts .k { font-size:12px; color:#6B6880; font-weight:600; }
  .spec .facts .v { font-size:12px; color:#16151F; font-weight:600; margin-top:2px; }
  .spec .ro { flex-basis:100%; font-size:12px; color:#6B6880; }
  ...
  .ctabs { display:flex; gap:30px; min-height:45px; align-items:center; padding-inline:20px;
    border-bottom:1px solid #EFEDF7; margin-bottom:16px; flex-wrap:wrap; }
  .ctabs button { font-family:inherit; font-size:14px; font-weight:500; color:#6B6880; background:none;
    border:none; border-bottom:2px solid transparent; padding:0 0 11px; cursor:pointer; white-space:nowrap; }
  .ctabs button.on { color:#6C5CE7; border-bottom-color:#6C5CE7; }
  ...
  .crmflat .crow:hover { background:#F0EEF9; }
  .crmflat .crow.sel:hover { background:#E9E6F4; }
  .kcard:hover, .ncard:hover { box-shadow:0 1px 2px rgba(0,0,0,.06); }
  .qpill:hover:not(.on), .ptab:hover:not(.on) { background:#E9E6F4; border-color:#D6D1E8; }
  .vtog button:hover:not(.on) { color:#16151F; }
  .rtabs button:hover:not(.on), .ctabs button:hover:not(.on) { color:#35333F; }
  .kebab:hover { background:#E9E6F4; }
  .lnk:hover { opacity:.7; }
  ...
  .inp:focus, .crmsel:focus { border-color:#6C5CE7; }
  ...
  .skel-row { display:grid; grid-template-columns:40px 2fr 1.1fr 1.5fr 1fr; gap:12px;
    padding:8px 20px 8px 12px; border-top:1px solid #EFEDF7; align-items:center; }
  .skel-row > i { display:block; height:12px; border-radius:4px; background:#EFEDF7;
  ...
  .tgtrow { grid-template-columns: 40px 1.8fr 1fr 1.9fr 1.5fr .55fr .85fr; border-top:1px solid #EFEDF7; }
  ...
  .fig { display:flex; gap:12px; font-size:12px; color:#35333F; }
  .crow .c-num { text-align:center; font-size:14px; font-weight:450; color:#16151F; font-variant-numeric:tabular-nums; }
  ...
  .thead-narrow { display:none; align-items:center; gap:10px; padding:12px 16px; background:#F0EEF9;
    border-block-end:1px solid #EFEDF7; font-size:12px; font-weight:600; color:#6B6880; }
  ...
  @media (max-width: 939px) {
    ...
    .crow .c-num .lbl-ph { display:inline; color:#6B6880; font-weight:600; }
    ...
    .crow .cedge   { display:block; position:absolute; inset-inline:0; inset-block-end:0; height:3px;
      background:#EFEDF7; }
    .crow .cedge > i { display:block; height:100%; background:#6C5CE7; }
...
/* The five states, declared ONCE and in board order. campPerfState selects from this table and the
   ...
   thing and the row chip another — the drift the check:crm assertion exists to catch. */
var CRM_PERF = [
  { key: "test",       label: "تجريبية",       cls: "c-warn", dot: "#7A5600" },
  { key: "noaudience", label: "بلا جمهور",     cls: "c-grey", dot: "#A9AEBE" },
  { key: "unsent",     label: "لم تُرسل بعد",   cls: "c-grey", dot: "#A9AEBE" },
  { key: "replied",    label: "فيها ردود",     cls: "c-ok",   dot: "#12633F" },
  { key: "silent",     label: "بلا ردود بعد",  cls: "c-blue", dot: "#5A4BD6" }
];
...
function crmControlBar(nShown, nTotal) {
  ...
  h += '<button class="btn btn-ghost" style="height:32px;padding:0 12px;border-radius:6px;font-size:12px;" onclick="exportCampaigns()">' + ic("doc", 15) + ' تصدير CSV</button>';
  ...
  if (crmView !== "list") {
    ...
    h += '<div style="flex-basis:100%;font-size:12px;color:#6B6880;padding-top:2px;">' +
      esc(kd[1]) + " — " + esc(kd[3]) +
...
function crmRow(c, st) {
  ...
  return '<div class="trow km krow crow' + (on ? " sel" : "") + '" role="link" tabindex="0"' +
    ...
    '<div class="c-name" style="display:flex;align-items:center;gap:12px;min-width:0;"><span role="img" aria-label="' + (isTest ? "حملة تجريبية" : "حملة فعلية") + '" style="width:9px;height:9px;border-radius:999px;flex:none;background:' + (isTest ? "#D6D1E8" : "#6C5CE7") + ';box-shadow:0 0 0 3px ' + (isTest ? "rgba(208,213,221,.28)" : "rgba(31,122,115,.16)") + ';"></span>' +
    '<div style="min-width:0;"><div style="font-size:14px;font-weight:500;color:#16151F;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' + esc(c.name) + '</div></div>' +
    '<span style="font-size:12px;color:#6B6880;flex:none;">' + fmtD(c.created_at) + '</span></div>' +
    '<div class="c-meta">' +
    '<div class="c-prod" style="font-size:12px;color:#35333F;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;min-width:0;">' + esc(c.product || "—") + '</div>' +
    '<div class="c-state" style="display:flex;align-items:center;gap:7px;white-space:nowrap;"><span style="width:6px;height:6px;border-radius:999px;flex:none;background:' + ps.dot + ';"></span><span style="font-size:14px;color:#35333F;">' + ps.label + '</span></div></div>' +
    '<div class="c-fig fig">' +
    ...
    '<div class="c-prog">' +
      (prog === null
        ? '<span style="font-size:12px;color:#6B6880;">لا جهات استهداف</span>'
        : !crmWasSent(st)
        ? '<span style="font-size:12px;color:#6B6880;">لم تُرسل بعد</span>'
        : '<div class="prog" style="flex:1;height:6px;background:#EFEDF7;border-radius:999px;overflow:hidden;"><i style="display:block;height:100%;width:' + prog + '%;background:#6C5CE7;border-radius:999px;"></i></div><span style="font-size:12px;font-weight:600;color:#6B6880;flex:none;font-variant-numeric:tabular-nums;">' + fmtN(prog) + '٪</span>') +
    '</div>' +
    '<div class="c-act" style="text-align:center;"><button class="kebab" title="' + actTitle + '" aria-label="' + actTitle + '" onclick="event.stopPropagation();setCampClass(' + c.id + ',' + (isTest ? "false" : "true") + ')">' + ic("target", 17, isTest ? "#D6D1E8" : "#6C5CE7") + '</button></div>' +
...
function crmHeaderRow(withSelectAll, allOn, nOver, nTotal) {
  var selAll = withSelectAll && nOver > 0
    ? ' <span class="lnk" onclick="event.stopPropagation();crmSelectAllMatching()" style="color:#6C5CE7;font-weight:600;cursor:pointer;font-size:12px;">تحديد المطابقين (' + fmtN(nTotal) + ')</span>'
    : "";
  ...
  return '<div class="crow thead-wide" style="padding:8px 20px 8px 12px;background:#fff;border-bottom:1px solid #EFEDF7;font-size:12px;font-weight:500;color:#6B6880;">' +
      ...
      '<div class="c-fig fig" style="font-weight:600;color:#6B6880;">' +
        '<div class="c-num" style="color:#6B6880;font-size:12px;">الجمهور</div>' +
        '<div class="c-num" style="color:#6B6880;font-size:12px;">مشاهدة</div>' +
        '<div class="c-num" style="color:#6B6880;font-size:12px;">ردود</div>' +
      '</div>' +
...
function crmEmptyList() {
  if (campQ.trim()) return '<div style="padding:44px;text-align:center;color:#6B6880;font-size:14px;line-height:1.9;">لا حملة تطابق «' + esc(campQ.trim()) + '».<br><span style="color:#6B6880;">امسح البحث أو جرّب تبويبًا آخر.</span></div>';
  return '<div style="padding:44px;text-align:center;color:#6B6880;font-size:14px;">لا حملات في هذا التبويب</div>';
}
...
function crmGroupView(withStAll) {
  ...
  if (g.order.length <= 1) {
    return '<div class="sparse rise">' + ic("eye", 16, "#6C5CE7") +
      ...
      '<span class="lnk" onclick="crmSetView(&quot;list&quot;)" style="color:#6C5CE7;font-weight:600;cursor:pointer;">→ عد إلى القائمة</span></div></div>' +
  ...
  g.order.forEach(function (k) {
    ...
    h += '<div class="tblwrap crmflat rise" style="margin-bottom:14px;">' +
      '<div style="display:flex;align-items:center;gap:9px;padding:13px 18px;border-bottom:1px solid #EFEDF7;background:#F0EEF9;">' +
      '<span style="font-size:14px;font-weight:600;color:#16151F;">' + esc(k) + '</span>' +
    ...
    if (!rows.length) h += '<div style="padding:26px;text-align:center;color:#6B6880;font-size:12px;">لا حملات في هذه المجموعة</div>';
    h += '</div></div>';
    if (over > 0) h += '<div class="tfoot"><span style="color:#7A5600;font-weight:600;">تُعرض ' + fmtN(rows.length) + " من " + fmtN(g.by[k].length) + " في هذه المجموعة.</span></div>";
    h += '</div>';
  });
  return h;
}
  ...
  g.order.forEach(function (k) {
    ...
    rows.forEach(function (x) {
      ...
      if (!st.targeted) {
        ...
        if (st.failed > 0) foot += '<span class="ksep">·</span><span class="kl" style="color:#8E2A27;">تعذّر ' + fmtN(st.failed) + '</span>';
      }
    ...
    if (g.by[k].length > rows.length) h += '<div style="font-size:12px;color:#7A5600;font-weight:600;padding:4px;">تُعرض ' + fmtN(rows.length) + " من " + fmtN(g.by[k].length) + '</div>';
    h += '</div>';
  });
  h += '</div>';
  return h;
...
function vKmonCrm(d) {
  ...
  if (!campaigns.length) {
    return h + '<div class="empty" style="padding:60px 20px;"><div class="ic"><span></span></div><div class="t">لا حملات بعد</div><div class="s">أطلق أول حملة من <a href="#aimkt" style="color:#6C5CE7;font-weight:600;">إنشاء حملة</a> — كل إطلاق يظهر هنا بلوحته وأرقامه الحية.</div></div>';
  }
...
function crmSpecStrip(camp, st) {
  ...
  var body = msg
    ? '<div class="bub2' + (crmMsgOpen ? "" : " clamp") + '" id="crmmsg">' + esc(msg) + '</div>' +
      '<div id="crmmsgmore" style="margin-top:7px;' + (crmMsgOpen ? "" : "display:none;") + '"><span class="lnk" onclick="crmToggleMsg()" style="color:#6C5CE7;font-weight:600;font-size:12px;cursor:pointer;">' + (crmMsgOpen ? "طيّ النص" : "عرض النص كاملًا") + '</span></div>'
    : '<div style="font-size:12px;color:#6B6880;padding:8px 2px;">لم يُحفظ نص هذه الحملة.</div>';
...
function vKmonDetailCrm(id, d) {
  ...
  var h = '<a href="#kmon" style="display:inline-flex;align-items:center;gap:6px;font-size:12px;font-weight:600;color:#35333F;text-decoration:none;margin-bottom:14px;">→ كل الحملات</a>' +
  ...
  h += '<div class="card rise" style="display:flex;gap:26px;flex-wrap:wrap;align-items:center;">' +
    ...
    '<div style="font-size:14px;font-weight:500;color:#16151F;margin-top:6px;line-height:1.6;">' +
    ...
    [["نسبة المشاهدة", crmDeliveryRate(st.seen, st)], ["نسبة الردود", crmDeliveryRate(st.replied, st)], ["جهات مهتمة لكل ١٠٠", yieldPer100]]
      .map(function (x) {
        return '<div><div style="font-size:18px;font-weight:600;color:#16151F;font-variant-numeric:tabular-nums;">' +
          (x[1] === null ? "—" : fmtN(x[1]) + '<span style="font-size:12px;color:#6B6880;">٪</span>') +
          '</div><div style="font-size:12px;color:#6B6880;margin-top:3px;">' + x[0] + '</div></div>';
      }).join("") + '</div></div>';
  ...
  var moves = [];
  if (hotHere.length) moves.push(["ابدأ التواصل مع " + fmtN(hotHere.length) + " جهة تستحق المتابعة", "وسوم اهتمام مؤكدة، أو نية مرتفعة قرأها المساعد من نص المحادثة ولم تُسجَّل وسمًا بعد", "#12633F", "#E4F5EC", "interested"]);
  if (seenSilent.length) moves.push(["أعد استهداف " + fmtN(seenSilent.length) + " جهة شاهدت دون ردّ", "الاهتمام قائم، وأثر الرسالة غير واضح" + (topCause ? " وعالج «" + topCause + "»" : ""), "#7A5600", "#FFF5D6", "silent"]);
  if (notDelivered.length) moves.push([fmtN(notDelivered.length) + " لم تصلهم الرسالة", "تحقق من الأرقام، ثم أعد المحاولة لاحقًا", "#8E2A27", "#FBE7E6", "failed"]);
  ...
  if (moves.length) {
    var m0 = moves[0];
    h += '<div class="rise" style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;background:#fff;border:1px solid #EFEDF7;border-inline-start:3px solid #6C5CE7;border-radius:10px;padding:12px 16px;margin-bottom:14px;">' +
      ic("spark", 16, "#6C5CE7") +
      '<span style="font-size:14px;font-weight:500;color:#16151F;">' + esc(m0[0]) + '</span>' +
      '<span style="flex:1;min-width:12px;"></span>' +
      (m0[4] ? '<button class="btn" style="padding:6px 12px;font-size:12px;border-radius:999px;color:#6C5CE7;background:#fff;border:1px solid #8B7BF5;font-weight:600;" onclick="crmGoFilter(&quot;' + m0[4] + '&quot;)">افتح هذه الفئة</button>' : "") +
      '<span class="lnk" onclick="crmSetDetailTab(&quot;next&quot;)" style="color:#35333F;font-weight:600;font-size:12px;cursor:pointer;">كل الخطوات (' + fmtN(moves.length) + ')</span>' +
      '</div>';
  }
  ...
  if (crmDetailTab === "perf") {
    ...
    var cards = [["جهات الاستهداف", st.targeted, "#5A4BD6", false], ["أُرسلت", st.sent, "#5A4BD6", false],
      ["وصلت", st.delivered, "#8B7BF5", false], ["شوهدت", st.seen, "#8B7BF5", true],
      ["ردّوا", st.replied, "#5A4BD6", true], ["جهات مهتمة", st.interested, "#1E9E63", true]];
    ...
    '<div style="font-size:12px;color:#6B6880;margin-top:10px;">«شوهدت» = قُرئت أو ردّت — أي إشارة مؤكدة أن الرسالة وصلت لعين العميل.</div>';
    return h;
  }
  ...
  if (crmDetailTab === "next") {
    ...
    h += '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(250px,1fr));gap:12px;">' +
      moves.map(function (m) {
        return '<div style="background:#fff;border:1px solid #EFEDF7;border-inline-start:3px solid ' + m[2] + ';border-radius:10px;padding:14px 16px;">' +
          '<div style="font-size:14px;font-weight:500;color:#16151F;">' + esc(m[0]) + '</div>' +
          '<div style="font-size:12px;color:#35333F;margin-top:5px;line-height:1.8;">' + esc(m[1]) + '</div>' +
          (m[4] ? '<div style="margin-top:10px;"><button class="btn" style="padding:6px 12px;font-size:12px;border-radius:999px;color:#6C5CE7;background:#DDD6F7;border:1px solid #8B7BF5;font-weight:600;" onclick="crmGoFilter(&quot;' + m[4] + '&quot;)">افتح هذه الفئة</button></div>' : "") +
          '</div>';
      }).join("") + '</div>';
    return h;
  }
  ...
  h += '<div style="overflow-x:auto;" class="ms-scroll"><div style="min-width:900px;">' +
    (shown.length ? crmTargetRows(shown, cwin) : '<div style="padding:30px;text-align:center;color:#6B6880;font-size:12px;">لا نتائج</div>') +
    '</div></div></div>';
...
function crmTargetHeader(allOn) {
  return '<div class="crow tgtrow thead-wide" style="padding:8px 20px 8px 12px;background:#fff;border-bottom:1px solid #EFEDF7;font-size:12px;font-weight:500;color:#6B6880;">' +
...
function crmTargetRows(shown, cwin) {
  ...
  shown.forEach(function (r) {
    ...
    var extra = tags.length > 2 ? '<span style="font-size:12px;color:#6B6880;">+' + fmtN(tags.length - 2) + '</span>' : "";
    out += '<div class="trow km krow crow tgtrow' + (on ? " sel" : "") + '" onclick="location.hash=&quot;customer/' + esc(c.phone) + '&quot;">' +
      ...
      '<div class="c-name" style="display:flex;align-items:center;gap:10px;min-width:0;">' +
        ...
        '<span style="font-size:12px;color:#6B6880;flex:none;direction:ltr;">' + esc(c.phone) + '</span></div>' +
      ...
      '<div style="font-size:12px;color:#35333F;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;min-width:0;">' +
        (ci.next_action ? '<span style="color:#6C5CE7;">← ' + esc(ci.next_action) + '</span>' : esc(last ? clip(last.text, 60) : "—")) + '</div>' +
      '<div style="font-size:12px;color:#6B6880;white-space:nowrap;">' + (last ? fmtT(last.ts) : "—") + '</div>' +
      '<div style="text-align:center;"><span class="lnk" style="font-size:12px;color:#6C5CE7;font-weight:500;" onclick="event.stopPropagation();openConvo(&quot;' + esc(c.phone) + '&quot;)">المحادثة ←</span></div>' +
    '</div>';
  });
  return out;
}
...
function crmPaintCrumb() {
  ...
  if (act) {
    act.innerHTML = '<a href="#aimkt" class="btn" style="text-decoration:none;color:#fff;background:#6C5CE7;">' +
